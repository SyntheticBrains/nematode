...
- **Actively developed research software, single maintainer.** Phases 0–5, Phase 6**a** and Phase 7 of the [roadmap](docs/roadmap.md) are complete ([Phase 6a synthesis](docs/experiments/logbooks/037-phase6a-synthesis.md), [Phase 7 synthesis](docs/experiments/logbooks/069-phase7-synthesis.md)); **Phase 6b remains pending**. **Phase 7 closed as a SPLIT**: its flagship question — does the wild-type wiring become load-bearing under a biologically plausible learning rule — could not be answered, because no rule in that family writes this connectome to any benefit; e-prop reaches competence with the chemical matrix **frozen**, and every arm that writes it does worse. What the phase delivered instead is that the wiring is **learning-speed-relevant under gradient descent** (the wild type reaches competence 23–55% sooner than a degree-preserving rewired null, with the standing condition that rewiring varies with initialisation in those panels — **whose pairing half Phase 8's A.1 partially discharged**: no dissolution was detected under either definition of a shared initialisation and survival was established on five of eight readings, with three unresolved, and the thermal cell's magnitude replicated at only 35% of its committed size; and [A.2](docs/experiments/logbooks/071-operating-point-surface.md) found it holds at a settling depth of at least four hops under PPO, abolished one hop shallower, with the reading learner putting the null ahead at the same point), and that it is **legible to a small local readout only at a particular readout width and learning rate**. **Phase 8** (roadmap v4.3, *ground, then embody*) has shipped its first half: **8a is complete** ([8a synthesis](docs/experiments/logbooks/076-8a-synthesis.md)). Block V's learning-speed advantage holds with its conditions named — no dissolution under a shared initialisation, depth-critical, unmoved by measured synaptic weights — and about half of its `auc_success` lead turned out to come from the rewired null's gap junctions; against a null that keeps the wild type's gap junctions it is +0.025 `auc_success` and +235 episodes to competence. Next is **8b**: the anatomical motor-to-muscle map into a two-dimensional body, read against a null that differs from the wild type only in chemical placement — *C. elegans* only, no cross-species, multi-agent or evolutionary work. Cross-species transfer to *P. pacificus* is deferred with 7b; Phase 6b (NEAT topology search) is deferred behind environment vectorisation.
...
- **Engineering.** Python 3.13 only; 4,000+ tests in four tiers (unit, slow integration, CLI smoke, nightly end-to-end benchmarks); Pyright-typed and Ruff-linted (`select = ALL`). Tagged releases are on [GitHub Releases](https://github.com/SyntheticBrains/nematode/releases) and summarised in [CHANGELOG.md](CHANGELOG.md); internals change between them.
...
|---|---|
| [docs/roadmap.md](docs/roadmap.md) | Vision, phase history, exit criteria, decision gates and the Phase 8 plan |
...
- **Phase 8 substrate and body**: reversal and a proprioceptive channel in the continuous environment; the anatomical motor-to-muscle readout and a 2D rod-chain body (ElegansBot-class); ingesting measured synaptic weights on the Cook edges with provenance.
