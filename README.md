...
- **Evidence discipline.** Every experiment series is a numbered [logbook](docs/experiments/README.md) with pre-registered hypotheses, paired-seed statistics (n ≥ 8, Wilcoxon, bootstrap CIs, BH-FDR) and the configs, seeds and artifacts needed to reproduce it. Negative results and STOP verdicts are first-class. Changes go through [OpenSpec](https://github.com/Fission-AI/OpenSpec) proposals under [`openspec/`](openspec/).
- **Engineering.** Python 3.13 only; ~3,600 tests in four tiers (unit, slow integration, CLI smoke, nightly end-to-end benchmarks); Pyright-typed and Ruff-linted (`select = ALL`). Tagged releases are on [GitHub Releases](https://github.com/SyntheticBrains/nematode/releases); internals change between them.
...
uv run ./scripts/run_simulation.py --config configs/scenarios/multi_agent_foraging/mlpppo_medium_5agents_social_oracle.yml --theme pixel --runs 10
