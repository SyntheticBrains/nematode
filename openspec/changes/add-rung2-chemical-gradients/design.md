...
What is **locked**: a Linear-Nonlinear stage in which the chemosensory signal is coded **relative to a slowly-tracked background**, not as a static squash. A leaky-integrator background `B_t = (1−α)·B_{t−1} + α·C_t` (adaptation timescale τ via α) supplies the background; the readout is **biphasic relative coding** (ON when the signal rises above background, OFF when it falls below). **Why:** the current tanh has no adaptation at all; plain log-concentration is the roadmap's named under-powered special case. **Reuse pattern, not API:** `STAMBuffer` (`agent/stam.py:244`) exposes `record` / `compute_temporal_derivative` / `get_memory_state` but **no public running-background/baseline API** (its weighted mean lives privately inside `get_memory_state` over the private `_scalar_history` deque). So the background tracker is a **small shared leaky-integrator utility** that follows STAM's exponential-decay pattern (and may be factored out of STAM), not a direct reuse of a STAM baseline method.
...
2. **Fold-change detection on the derivative channel** — normalize the temporal derivative by concentration, `(dC/dt)/(C + ε) ≈ d(log C)/dt` (the `+ ε` is **load-bearing**: `C ≈ 0` is common far from a source, so the raw `(dC/dt)/C` is singular — regularise with `C + ε`, or normalise by the tracked background `B + ε`), applied to the `dC/dt`/turning channel that klinotaxis actually acts on. This is the more literature-faithful reading of the hyper-Weber result (Kato et al. 2014; Levy & Bargmann 2020): the *derivative response scales with background*, it is not only an instantaneous magnitude contrast.
...
## Deviations from plan (as-built, 2026-06-08)

Recorded so the change reflects what shipped (logbook 028). The spec-level behavioural contracts (D1–D7) all hold; these are implementation-location / scope refinements made during the build:

- **Adaptive transform lives in the agent sensory-assembly step, not the `brain/modules.py` cores (refines D2).** D2 said "at the `brain/modules.py` core-feature stage / a dedicated adaptive-sensor step." As-built it is the *dedicated step*: `agent._compute_temporal_data` reshapes `result["food_concentration"]` / `result["food_dconcentration_dt"]` (after the STAM derivative, before they become `BrainParams`), so the duplicated pure `*_klinotaxis_core` functions stay untouched and there is a **single clean chokepoint** (better than threading the stateful background into ~9 cores). The `klinotaxis-sensing` contract — the strength/binary fields carry the adaptive output when enabled — is satisfied from upstream.
- **Channel interaction is encoded in the single `adaptive_chemosensor_readout` enum.** `fold_change` ⇒ derivative/turning channel; `contrast` ⇒ strength channel; `log` ⇒ strength baseline. One explicit field rather than a separate interaction knob — still satisfies the spec's "channel-interaction is explicit" scenario.
- **Food channel only this tranche.** The transform is wired to the food chemosensory channel; pheromone / CO₂-chemo channels are **not** wired (those behaviours are deferred — Decision 5). The spec's "chemosensory channels (food + pheromone/CO₂ where active)" is met for the active channel; extending to pheromone/CO₂ is a follow-up.
- **No `validate_sensing_config` interlock added.** Fold-change consumes the STAM-derived derivative, so a meaningful adaptive signal relies on STAM/klinotaxis being enabled (the sample config does). With STAM off the derivative is `0` and fold-change is inert rather than erroring — acceptable graceful degradation; an auto-enable interlock is a possible follow-up.
- **Float-source type confinement realised via runtime, not annotation (refines D4).** `self.foods` keeps its `list[tuple[int, int]]` annotation (widening it rippled into PathTracker / rendering / grid Manhattan distance); the continuous-2D subclass stores floats at runtime via the overridden `_generate_food_candidate`. The analytic field getters (`get_food/predator/temperature/oxygen/pheromone_*` + their delegated field methods) widened to the float `Position` alias (int is assignable to float — grid callers unaffected). The two cell-indexing reporting/render sites (`food_history`, grid-render goals) snap floats to cells; the int-typed `SimulationResult` schema is unchanged.
- **Fidelity renderer (group 5) deferred** as a non-gating seed (logbook 028 § Deferrals); the existing grid render was made float-safe so continuous runs work headless. Matplotlib logbook figures deferred with it — the quantitative validation tables are the load-bearing evidence.
