...
- [x] 3.4 Config surface on `SensingConfig` (`config_loader.py:734`): enable flag, readout mode, channel-interaction mode (explicit), `α`/`τ`, `ε`; extend `validate_sensing_config` (`:861`) for any interlocks; sample config(s) on the continuous substrate.
