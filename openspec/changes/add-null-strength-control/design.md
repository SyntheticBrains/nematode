...
hard350 arms, and A.2's reading centre. The 4 new configs are the `_rewired_chemical_null` arms,
learning and frozen, each derived from its full-null parent with `wiring` changed and nothing else.
...
**The proxy is conservative.** This interaction differences two null arms at one seed, and the
wild-type noise cancels, so its spread is probably smaller. The launch record states the exact
figures from the committed CSVs.
...
|---|---|---|
| `no_move` | **chemical** | Against a null holding the wild type's gap junctions (placement and strength) and autapses, the wiring gap moves by less than the minimum. Block V's advantage is in the chemical wiring *given those held in place*. |
...
**The reading learner's reference effect is negative**: the null leads at A.2's centre. Orientation
is unchanged, since a positive interaction means a better wild-type position. On that learner,
`move_null` means the null's lead grows against the chemical null.
