...
- [ ] 0.3 **B0.2 Sparse-signal ablation**. Implement a sensor-module variant `predator_mechanosensation_klinotaxis_sparse_fix` that injects `predator_distal_concentration` into the `predator_contact_intensity` field when `predator_contact_zone == ContactZone.NONE`. Sub-steps:
