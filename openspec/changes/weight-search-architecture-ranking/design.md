...
Per-cell raw artefacts (CSVs, per-cell summary tables, plots) that the logbook references SHALL live under `docs/experiments/logbooks/supporting/0XX-weight-search-architecture-ranking/`. The `tmp/` scratchpads (`tmp/evaluations/weight-search-architecture-ranking/...` for Phase 4 and a separate Phase 0 scratchpad) are working forensics that don't persist across machine state; they SHALL NOT be referenced from the logbook.
