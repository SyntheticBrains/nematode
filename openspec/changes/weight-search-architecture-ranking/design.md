...
The C3 primary cells run at **1000 episodes** (not the 500 of Phase 0); the four MUST C3 cells use this 1000ep budget for env parity (MLPPPO/connectome are merely over-trained — harmless). The from-scratch Phase-4.5 promoted arms (spiking, equivariant-quantum) instead run a larger budget — up to **4000ep** — for their long dead-exploration warm-ups; this does NOT break the comparison because the ranked metric is the post-convergence plateau (`post_convergence_success_rate`), which measures each arm at its own converged plateau rather than at a fixed episode count, so a uniform episode budget across arms is not required (only that every arm has reached its plateau). The normative budget requirement lives in the `architecture-comparison-protocol` spec § "Convergence-Aware Budget and Plateau-Performance Metric" (the budget is set to the slowest-converging architecture's convergence point); this paragraph is the supporting rationale + the pre-flight evidence for the 1000ep value. The C1/C2 curriculum smokes (n=1) stay at 500ep. The connectome may need MORE than 1000ep (its C2 was only 16% at 500ep — slowest-converging architecture); the connectome C3 pre-flight read + any budget bump is recorded in the Phase 2 scratchpad and the compute-budget subsection below. **Seed count (T4.0b)**: floor n ≥ 4 per Phase 5 inheritance; raise to n = 8 on a cell only if its primary-metric SE exceeds 0.5× the gate threshold at n = 4 (M4.5 precedent).
...
### Convergence definition + plateau-performance metric + adaptive budget (Phase 4/5 methodology)
...
**The comparison's ranked metric (realised).** The Phase 5 ranking is on **`post_convergence_success_rate`** — the full-clear rate over the detected post-convergence plateau (`detect_convergence`: earliest 10-run window with success-variance < 0.05 and mean > 0.5, averaged to the run end), NOT a fixed last-N window. A fixed window was rejected because the arms have very different warm-up lengths (the from-scratch spiking + quantum arms have long dead-exploration warm-ups on this lethal cell), so a last-25 window would mis-rank the slow-igniting arms. **Sample efficiency** (how fast — episodes/generations to 90% of plateau) is reported *descriptively* (the per-500-ep clear-rate trajectory in the logbook shows the warm-up shape), NOT as a computed per-architecture metric feeding the ranking — a deliberate realised simplification, with the asymptotic plateau ranking as the load-bearing result. See the `architecture-comparison-protocol` spec § "Convergence-Aware Budget and Plateau-Performance Metric".
...
Phase 4 produced the four-MUST integrated-C3 ranking (connectome, MLP-PPO, LSTM-PPO, FeedforwardGA). Before publishing, this gate decides — per Decision 3 (the architecture-promotion gate) — whether to promote any SHOULD/MAY architecture from [`phase6-tracking/design.md § Decision 4`](../phase6-tracking/design.md) into the comparison. Each verdict cites Decision 3's three criteria: **(a)** per-architecture compute fits the remaining budget, **(b)** roadmap-relevance, **(c)** whether Phase 4 already settles the headline.

| Candidate | Decision 4 class | Verdict | Realised |
|---|---|---|---|
| Quantum | SHOULD | **GO** | equivariant-quantum C3 n=8 = 86.0% + 4 controlled attribution arms |
| Spiking | SHOULD | **GO** | spiking-PPO C3 n=8 = 84.2% ± 1.1 |
| CfC (liquid) | *(not in Decision 4 table — see note)* | **GO** | CfC C3 n=8 = 84.4% ± 9.5 |
| Reservoir (QRH/CRH) | MAY | **SKIP** | deferred to T7 |
| Hybrid quantum-classical | MAY | **SKIP** | deferred to T7 |

**Quantum — GO.** (b) Directly addresses RQ4 ("does a quantum architecture confer advantage on this task?"), the single most roadmap-load-bearing question the four-MUST set cannot touch (none of the MUST families is quantum). (a) Classically simulated via the in-repo differentiable statevector — ~hours of compute, well within remaining budget. (c) Phase 4 leaves RQ4 entirely unsettled, so this is the load-bearing promotion. The arm shipped its C3 cell *plus* the four-arm controlled attribution (unstructured-quantum, thin- and rich-classical-equivariant, rich-classical-non-equivariant) that is the actual RQ4 payload — confirming **no quantum-circuit advantage and no significant symmetry effect** (Logbook 025). GO was vindicated: the raw rank (#1) without the controls would have been a misleading weak-baseline headline (the naive +24.6pp "quantum beats classical" delta was an artifact of a sub-MLP control).

**Spiking — GO.** (b) The neuromorphic bridge to Phase 7 L4 STDP (spiking's native plasticity rule); a recurrent adaptive-LIF substrate point the MUST set lacks. (a) Cheap (classical). (c) Adds a neuromorphic anchor to the temporal cluster. Decision 4 demoted spiking to SHOULD with the caveat "if it doesn't train cleanly, document and move on" — it **did** train cleanly (84.2% ± 1.1, the lowest variance of any arm), so the GO is recorded as vindicated rather than a documented-failure fallback.

**CfC (liquid) — GO, with a Decision-4 scope note.** CfC (`cfcppo`, a closed-form continuous-time / liquid recurrent network) is **not** in the Decision 4 SHOULD/MAY table. It was promoted because (a) it is already a registered brain and classically cheap, (b) it supplies a second strong temporal/recurrent baseline alongside LSTM — testing whether continuous-time dynamics buy anything over gated-recurrent on this cell — and (c) it sharpens rather than settles the headline (it landed in the top cluster at 84.4%, reinforcing the temporal-arm strength without flipping the no-quantum-advantage framing). **Scope note**: strictly, adding a family absent from the Decision 4 table is a Decision-4 amendment event (per that decision's mechanism). This grid pass is *partial* evidence (full Gate 3 closes at T7), so CfC enters here as a documented promotion, not a MUST; the Decision 4 table should absorb CfC (as a SHOULD/MAY liquid-network row) when it is next amended at the T7 / T9 synthesis, where its continuous-substrate behaviour is the load-bearing test. Recorded here so the scope expansion is not silent.

**Reservoir (QRH/CRH) — SKIP.** (c) The headline is already settled where a reservoir row could move it: the top cluster is a four-way statistical tie with **no quantum advantage** under controlled attribution, and Phase 2 preserved reservoir variants only at *low absolute performance* with a narrow pursuit-specific edge — a reservoir row would land below the cluster and not flip the framing. (b) Less roadmap-load-bearing than the quantum/spiking promotions; its RQ4-adjacent value (a quantum-reservoir point) is already covered by the equivariant-quantum arm's controlled attribution. Deferred to Tranche 7's opportunistic SHOULD/MAY evaluation per [`phase6-tracking/design.md § Decision 4`](../phase6-tracking/design.md) (the Tranche 7 row: "SHOULD/MAY architectures evaluated opportunistically").

**Hybrid quantum-classical — SKIP.** Same headline-impact logic (c): the question a hybrid row would probe ("does mixing quantum + classical layers help?") is answered in the negative direction by the equivariant-quantum controlled attribution — a hybrid row adds cost without changing the no-quantum-advantage finding. (b) Phase 2's hybrid SOTA finding survives as a baseline reference and does not need re-running on the grid substrate to inform the T7 continuous-substrate question. Deferred to Tranche 7 opportunistic evaluation.

**Net.** Three GO (quantum, spiking, CfC), two SKIP (reservoir, hybrid). The promoted arms turned the four-MUST ranking into the published seven-family ranking (Logbook 025); the two SKIPs are deferred to T7 with their rationale on record, satisfying Decision 3's anti-silent-scope-change intent.
...
5. **Whether to amend [`phase6-tracking/design.md § Decision 6`](../phase6-tracking/design.md) to reflect this change's BH-FDR choice as the canonical T4+T7 strategy.** **Settled — yes.** The amendment landed at Task 1f.2 (2026-05-30): Decision 6's G3.a now reads "BH-FDR within-pass," explicitly resolving the prior "default Holm-Bonferroni, deferred to T4 planning" note (there was no separate Holm-Bonferroni default statement elsewhere in Decision 6 to flip). Confirmed post-T4-realisation at Task 6.10 (2026-06-03): the realised T4 pass shipped BH-FDR; G3.a's T7 inheritance stands. The roadmap's Mid-phase-gate section names no MCC default, so no roadmap edit was needed.
