...
- **New configs**: ~12 new small-klinotaxis configs (4 architectures × C1/C2/C3) + connectome-specific predator/thermotaxis configs + Phase 0 ablation configs. PPO-family configs live under `configs/scenarios/` (with the new `foraging_predator_thermal/` subdirectory for combined cells per design.md Decision 8); the GA combined config lives under `configs/evolution/` because GA configs require an `evolution:` block and are launched via `scripts/run_evolution.py`. Phase 0 ablation configs are one-shot evaluation artefacts under `tmp/evaluations/weight-search-architecture-ranking/phase-0/` (not in `configs/`).
