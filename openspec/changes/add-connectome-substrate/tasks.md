...
- [x] 4.3 For each neuron, attach a neurotransmitter when known (Glutamate, GABA, Acetylcholine, Serotonin, Dopamine, etc.) from cect's neurotransmitter data and `docs/nematode_biology.md` § Neurotransmitter Systems. Default `None` where uncertain. **Scope decision**: shipped as `None` for all 302 entries in this initial L0 table. The `neurotransmitter: str | None` slot in the `Neuron` data model is present and consumers can iterate it; populating actual NT identities is non-blocking for L0 (no downstream consumer depends on NT data at T1 — sensor/motor routing comes from `cell_class`, not NT). A future enrichment commit will populate well-documented NTs (Glutamate for ASE/AWC, GABA for VD/DD, Acetylcholine for VA/VB/DA/DB, Serotonin for NSM/HSN, Dopamine for ADE/PDE/CEP). The slot is forward-compatible — populating it later doesn't change the API contract.
