...
- **D16** operating-point discipline: no Phase 8 contrast at an unswept pin; A.2 sweeps `plasticity_rate`, readout width, `forward_pass_depth`, `initial_log_std`, `trace_decay` on the reading learner, one-factor-at-a-time first, reported as a sensitivity surface. *(Amended 2026-09-20 — see below.)*
...
*Amendment 2026-09-20 (D16, matching roadmap edit in the same PR; raised at this change's own PR review).* As ratified, D16 defined A.2 on the reading learner alone, while B.1 and B.2 both register **PPO** arms at "A.2's swept point" — so the requirement as written blocked the contrasts the plan depends on, because PPO had swept nothing. The pins are not all the same kind: `plasticity_rate` and `trace_decay` are the plasticity rule's, while readout width, `forward_pass_depth` and `initial_log_std` are substrate and policy settings both learners carry. **A.2 therefore gains a second half** — PPO across the three shared pins at a reduced grid — and the obligation is stated per learner and per pin the learner has. **A.1 is exempted explicitly**, because it re-runs the committed block-V operating point by design; it carries its pins as a standing condition and is re-read if A.2's PPO half moves the sign. This is a scope increase for A.2, and it is the right one: Logbook 069 lists `forward_pass_depth` as never varied and `initial_log_std` as swept only by hand under the rule, so the phase's most exposed claim — a PPO result — currently rests on two pins nobody has moved.
...
05. **Byte-identical-when-off** for every new mechanism (shared-init modes, measured-weight priors, dynamics, reversal, proprioception, the muscle readout, the body) — **and an arm must be shown to change only what it names, at the point the arms diverge rather than only at construction** (protocol principle 6's 2026-09-21 clause). A manipulation that consumes a shared mutable resource by a different amount is not a one-variable manipulation however well its parameters match: A.1's `dense_mask` drew 91,204 values from a generator the rollout buffer also uses, moving PPO's minibatch order as well as the weights, and every parameter check passed because the divergence only exists once training starts.
