...
- [x] **B.1c The 2×3** — **met 2026-09-25** ([Logbook 073](../../../docs/experiments/logbooks/073-measured-prior-contrast.md), `add-measured-prior-contrast`, 960 runs). **Under PPO the measured weights do not move the wiring effect; under the reading learner the panel cannot tell.** PPO (fan-in draw, pooled readout, with its depth and initial noise taken from A.2's `edge_order` surface and readout width unresolved there): the measured-versus-random interaction is +0.013 [−0.001, +0.027] `auc_success`, inside the 0.037 minimum on both sides, and the placement contrast's upper bound (+0.040) just crosses it — verdict `null_placement_unresolved`. Reading learner: the null's lead narrows from −0.109 to −0.026, an interaction of +0.082 [−0.002, +0.157] spanning the 0.140 minimum — verdict `unresolved`. Every arm beat its floor at every prior; no Lee failure; the reading substrate's drift was 0.0. Reported beside the primary: under PPO the wild type leads at all three priors (+0.078, +0.091, +0.070, each interval excluding zero), so block V's advantage does not depend on the weights being random on the covered edges. The reading learner's unresolved interaction is *deferred-with-destination* to S8a. Original scope: (own change): wiring {wild type, rewired null} × prior {random, measured,
...
### Carried from 8a, run early in 8b *(added 2026-09-27 at A.5)*
