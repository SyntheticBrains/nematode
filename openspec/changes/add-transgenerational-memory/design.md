...
The `tei_prior` stays constant across an episode (it is set once by `fitness.evaluate` post-decode and is not mutated during the step loop), which the call ordering at fitness.py:429 → fitness.py:460 enforces by construction. The normative version of this invariant — and the defensive `learn()` assertion — is in the lstm-ppo-brain spec's "PPO Sampling/Update Distribution Consistency" requirement; see `test_lstmppo_transgenerational_prior.py` for the concrete test.
