...
- [ ] 4.3 Extend BOTH the helper-level gates (`_inheritance_active()` or a new `_substrate_inheritance_active()`) AND the method-level dispatch in `_resolve_per_child_inheritance` ([loop.py:437-488](packages/quantum-nematode/quantumnematode/evolution/loop.py#L437-L488)) to handle `kind() == "transgenerational"`. The `_resolve_per_child_inheritance` branch for transgenerational SHALL return `(None, None, parent_id)` — same shape as `"trait"` (Baldwin), since TEI does not use per-child weight-IO paths. The TEI substrate is transported via the new worker-tuple elements (see evolution-framework spec "TEI Substrate Worker Transport"), NOT via this method.
...
- [ ] 5.3 In `quantumnematode/evolution/loop.py`: just before `optimizer.ask()` at the top of each generation, if `cfg.transgenerational is not None`, look up the schedule entry for the current generation, produce a per-gen `sim_config_for_gen = sim_config.model_copy(update={...})` Pydantic-v2 copy with `predators.enabled` set from the schedule entry's `pathogen_lawns_enabled`, and override the fitness invocation's `learn_episodes_per_eval` with `ppo_train_episodes`. The base `sim_config` SHALL NOT be mutated. Gate behind `if cfg.transgenerational is not None:` so the no-op path is byte-equivalent.
- [ ] 5.3b Extend `_evaluate_in_worker` worker tuple to 14 elements: append `(f0_substrate_path: Path | None, decay_factor: float | None, lineage_depth: int | None)` after the existing 11. For non-TEI strategies these SHALL be `(None, None, None)` preserving worker contract. Update `_evaluate_in_worker`'s tuple unpacking and the per-child dispatch in `EvolutionLoop.run()` that builds the tuple.
- [ ] 5.3c Inside `_evaluate_in_worker`: after constructing the agent/brain and BEFORE invoking the runner, if `f0_substrate_path is not None`, load the F0 substrate via `TransgenerationalMemory.load(f0_substrate_path)`, apply `inherit_from([f0], decay_factor)` `lineage_depth` times, and set `agent.brain.tei_prior = depth_n_substrate.logit_bias` via `hasattr(agent.brain, "tei_prior")` dispatch. If `hasattr` returns False, log a warning and proceed without setting. This is the single TEI-prior-set call site for both single-agent and multi-agent paths.
