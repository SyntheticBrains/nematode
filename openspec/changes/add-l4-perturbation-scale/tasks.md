...
- [ ] 1.1 Give `scripts/analysis/l4_rule_positive_control.py` a width parameter. `HIDDEN` stays the
...
- [ ] 1.2 `scripts/analysis/l4_perturbation_scale.py`: the S1 sweep over
...
- [ ] 1.3 Trials-to-criterion per seed: the first trial whose trailing 100-trial mean crosses the
...
- [ ] 1.4 Reachability: the analytic arm's gap fraction per width, and the rule's fraction normalised
...
- [ ] 1.5 The 1/N fit: OLS of `log2(trials)` on `log2(N)` over per-seed values, bootstrap CI over
...
- [ ] 2.1 Fifteen configs from the committed calibrated MLP food-only base: for each
...
- [ ] 2.2 Exact-key test for the family: each config differs from the committed base by the cell keys,
...
- [ ] 2.3 The S2 half of the harness: scan, plateau-tail mean foods through I.2's graded family,
...
- [ ] 2.4 Both effect minima enforced together — 1.0 foods of 20, **and** 10% of that width's own
  PPO-minus-frozen gap — with a downgrade to `below_min_effect` naming which minimum failed.
- [ ] 2.5 The capability gate: a width whose PPO arm misses the registered floor is reported
  `uninterpretable`, never as a null, and is excluded from the trend with that exclusion printed.
- [ ] 2.5b The declared alternative at the small-N end, **only if width 4 fails its capability gate**:
...
- [ ] 2.6 Per-width drift: relative weight distance from the frozen control, so starved-and-still is
  ...
  missing.
- [ ] 2.7 Tests for 2.3–2.6, including a synthetic fixture where one width is positive and another is
  uninterpretable, and one where the significance passes and both minima fail.

## 3. Pilot (disjoint seeds 101–104)

- [ ] 3.1 `launch.md` committed before anything runs.
- [ ] 3.2 Widths 4 and 64, all three arms, 24 runs: is the frozen arm off the floor, is the PPO arm off
...
- [ ] 3.4 If the platform has no room, apply the declared remedy — the committed C1 food-only cell
...
- [ ] 4.1 S1 first: it is minutes, it does not depend on S2, and its stop clauses gate the rest.
- [ ] 4.2 S2: 120 runs — three arms × five widths × eight seeds at 3000 episodes — run with
...
- [ ] 4.3 Per-seed CSVs, the per-width tables, both sweeps' verdicts and the combined verdict under
  `supporting/060-l4-perturbation-scale/`.

## 5. The record

- [ ] 5.1 Logbook 060: the confound table with all three platforms' dimensions; S1's slope, its CI, its
...
- [ ] 5.3 State plainly what the result does to the phase's existing negatives — re-read as
...
- [ ] 5.4 State what the result may not be cited as. In particular: a sweep topping out at 128 units
  says nothing measured about 302, and the derived budget is an extrapolation.

## 6. Close-out

- [ ] 6.1 `CHANGELOG.md`; the tracker's new **R** block with R.1, and R.2 (e-prop) carried as
  registered-not-run from [Logbook 059](../../../docs/experiments/logbooks/059-7a-shipment.md); the
  roadmap only if the reading changes.
- [ ] 6.2 Confirm no committed verdict was changed, and that the control's committed values reproduce
