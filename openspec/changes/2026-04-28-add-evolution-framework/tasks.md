...
- [ ] 3.4 Implement `EpisodicSuccessRate.evaluate()`. **Apply `seed` via `sim_config.model_copy(update={"seed": seed})` BEFORE `encoder.decode()`** (per Decision 3a). The brain's `__init__` calls `set_global_seed(self.seed)` ([mlpppo.py:170](packages/quantum-nematode/quantumnematode/brain/arch/mlpppo.py#L170)) which seeds both `np.random` and `torch.manual_seed`, AND `self.rng = get_rng(self.seed)` which seeds the brain's local Generator — so overriding `sim_config.seed` propagates to all three RNG sources via the brain's existing seeding infrastructure. **Do NOT call `torch.manual_seed(seed)` or `brain.rng = np.random.default_rng(seed)` directly** — those would either be no-ops (clobbered by the brain's constructor) or duplicates. Then `env = create_env_from_config(sim_config.environment, seed=seed)`, `agent = _build_agent(brain, env, sim_config)`, instantiate `FrozenEvalRunner()`, loop `episodes` calls to `runner.run(agent, sim_config.reward, sim_config.max_steps)`, count successes via `result.termination_reason == TerminationReason.COMPLETED_ALL_FOOD`, return `successes / episodes`. Success detection uses `TerminationReason.COMPLETED_ALL_FOOD` (codebase convention, see [experiment/tracker.py:304](packages/quantum-nematode/quantumnematode/experiment/tracker.py#L304)) — **NOT** a non-existent `result.episode_success` attribute. The `TerminationReason` import is `from quantumnematode.report.dtypes import TerminationReason` (NOT `quantumnematode.agent.dtypes`)
...
- [ ] 3.12 Unit test: `test_evaluate_overrides_sim_config_seed_before_decode` — instrument `encoder.decode` to record the `sim_config.seed` it received; instrument `create_env_from_config` to record its `seed` arg; call `evaluate(sim_config={seed: 99}, seed=42)`; assert the encoder saw `sim_config.seed == 42` (not 99) AND the env saw `seed=42`. This verifies `model_copy(update={"seed": seed})` is applied before decode and that the brain's `__init__` will therefore call `set_global_seed(42)` not `set_global_seed(99)`
- [ ] 3.13 Unit test: `test_evaluate_seed_differs_from_sim_config_seed_changes_fitness` — call `evaluate(sim_config={seed: 0}, seed=1)` then `evaluate(sim_config={seed: 0}, seed=2)`; assert the two fitnesses differ (proves the fitness function's `seed` parameter actually affects behaviour, not just sim_config.seed)
