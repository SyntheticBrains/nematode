...
- [ ] 2.1 Create `evolution/brain_factory.py` with `instantiate_brain_from_sim_config(sim_config: SimulationConfig, *, seed: int | None = None) -> Brain`. Body: extract `BrainConfig` via `configure_brain(sim_config)`; build an `overrides = {"weights_path": None}` dict (force `weights_path=None` so the genome is the sole weight source — see Decision 0); if `seed is not None`, add `overrides["seed"] = seed`; apply via `brain_config = brain_config.model_copy(update=overrides)`; then dispatch to `quantumnematode.utils.brain_factory.setup_brain_model()` with `brain_type` derived from `sim_config.brain.name`, `brain_config=patched`, `device=DeviceType.CPU` (hardcoded for evolution fitness eval), and other args extracted from `sim_config`. Match the seed-patching pattern from [scripts/run_simulation.py](scripts/run_simulation.py) — patch `BrainConfig.seed` (not `SimulationConfig.seed`). Default fallbacks for fields the YAML omits (e.g. `parameter_initializer_config=None`)
