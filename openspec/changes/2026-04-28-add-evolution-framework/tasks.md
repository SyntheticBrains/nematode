...
- [ ] 3.4 Implement `EpisodicSuccessRate.evaluate()`. **Apply `seed` to all three RNG sources at the top of the method** (per Decision 3a): (1) `torch.manual_seed(seed)`, (2) decode genome via encoder, (3) `brain.rng = np.random.default_rng(seed)` to override the brain's numpy Generator, (4) `env = create_env_from_config(sim_config.environment, seed=seed)`. Then `agent = _build_agent(brain, env, sim_config)`, instantiate `FrozenEvalRunner()`, loop `episodes` calls to `runner.run(agent, sim_config.reward, sim_config.max_steps)`, count successes via `result.termination_reason == TerminationReason.COMPLETED_ALL_FOOD`, return `successes / episodes`. Success detection uses `TerminationReason.COMPLETED_ALL_FOOD` (codebase convention, see [experiment/tracker.py:304](packages/quantum-nematode/quantumnematode/experiment/tracker.py#L304)) — **NOT** a non-existent `result.episode_success` attribute. The `TerminationReason` import is `from quantumnematode.report.dtypes import TerminationReason` (NOT `quantumnematode.agent.dtypes`)
...
- [ ] 3.12 Unit test: `test_evaluate_applies_seed_to_all_three_rng_sources` (instrument `torch.manual_seed`, `np.random.default_rng`, and `create_env_from_config` to record `seed` arg; call `evaluate(seed=42)`; assert all three saw `42`)
