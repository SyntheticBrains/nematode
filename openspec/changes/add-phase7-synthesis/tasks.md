...
- [x] 1.1 Every Phase 7 criterion gets exactly one of **met**, **unmet-with-reason**,
...
- [x] 1.2 **D4's learnable-gap-junction ablation: deferred-with-destination, narrowed** — B.6's
...
- [x] 1.3 **Co-primary biological validation: unreachable-with-reason** — both predictions are about a
  plastic wiring, and no rule that writes the wiring to a benefit exists. Recorded in both places.
- [x] 1.4 The three 7b SHOULD lines (dauer pathfinder, weight-transplant transfer, third behaviour)
...
- [x] 1.5 The 6a-preprint line corrected: the tracker records S.1 **cancelled** 2026-09-15, and the
...
- [x] 2.1 The D10 2×2's terminal reading against the claim-discipline bars: the primary (plastic
...
- [x] 2.2 The three shipped results, each with its standing condition **in the same sentence as the
...
- [x] 2.3 **The nulls are rate-robust**, and it is reported as the terminal reading's firmest part:
...
- [x] 2.4 The phase's negatives with their diagnosed causes — the rule programme's failure mode, the
  instrument block's finding, the ladder re-read — stated as diagnoses rather than as absence.
- [x] 2.5 The **unexamined-pins** table as a stated condition on every fixed-features result:
...
- [x] 2.6 The **wide readout's missing positive control** named, with the gates that stood in and why
  that is defensible here.

## 3. Block V's confound becomes a standing condition

- [x] 3.1 A dated note in [Logbook 065](../../../docs/experiments/logbooks/065-wiring-fresh-rewiring.md):
...
- [x] 3.2 The same condition in the tracker's block V entries and in the roadmap wherever the +35% /
  +55% figures are cited, in the same sentence as the claim.
- [x] 3.3 Why the control needs its own design decision — "the same initialisation" has no single
  meaning once the mask changes — so it opens a phase rather than closing one.

## 4. Reproducibility

- [x] 4.1 A reproducibility statement in the synthesis: re-derivable from git alone (every headline
...
- [x] 4.2 Name the concrete instance it already cost: `peak_action_density` uncomparable in L.1b's
  pilot because L.0's export had been deleted.

## 5. The roadmap and README terminal state (Z.2)

- [x] 5.1 Timeline Overview: Phase 7 → **✅ COMPLETE (SPLIT)**, the status cell delegating to the
  running-history section as it already does.
- [x] 5.2 The exit-criteria block flipped to terminal, every line carrying its status from task 1.
- [x] 5.3 Success Levels flipped: which level Phase 7 reached, stated against the criteria as written.
- [x] 5.4 `README.md`'s "Phases 0–6a are complete" line flipped, with Phase 7's shipped results named
  at the level of detail the README uses.

## 6. What Phase 8 opens on

- [x] 6.1 One inventory in the roadmap, in dependency order: **(a)** the init-vs-rewiring control,
  ...
  close.
- [x] 6.2 Each item carries what it inherits from Phase 7 — the condition or the result that motivates
  it — so the inventory is not a wish list.
- [x] 6.3 The carried SHOULD, L.2, appears in the inventory with its new precondition (the
  calibration rung) rather than as an unmarked leftover.

## 7. Bookkeeping

- [x] 7.1 Logbook 069 written, the experiments index row added, `CHANGELOG.md` entry.
- [x] 7.1a `docs/research/phase-protocol.md` principle 12 gains the **five-status vocabulary** and the
...
- [x] 7.2 The tracker: Z.1 and Z.2 ticked, L.2 settled as **carried** with its destination, and every
  status from task 1 recorded. No task left unmarked when the tracker archives.
- [x] 7.3 `uv run openspec validate --all --strict` (only `benchmark-management` may fail — a
