...
- [ ] 4.5.8 Unit test: `test_lineage_brain_type_uses_sim_config_brain_name_when_encoder_is_brain_agnostic` — construct an `EvolutionLoop` with a `HyperparameterEncoder` (so `encoder.brain_name == ""`) and a sim_config where `brain.name == "mlpppo"`; run one generation; read the resulting `lineage.csv` and assert every row's `brain_type` column is `"mlpppo"`, NOT `""`. Sibling test `test_best_params_json_brain_type_uses_sim_config_brain_name_when_encoder_is_brain_agnostic` — same setup, assert the written `best_params.json["brain_type"] == "mlpppo"`. Pairs with task 4.5.7. Locks S11's contract in
...
- [ ] 5.6 Subprocess test: `test_run_evolution_cli_fitness_learned_performance_requires_K` — invoke `--fitness learned_performance` against a config that has BOTH `hyperparam_schema:` populated (a minimal valid one — one entry suffices) AND `evolution.learn_episodes_per_eval=0`. The schema is required so guard (a) passes per task 5.3's ordering; guard (b) then fires. Verify exit code is 1 and stderr mentions the field

- [ ] 5.7 Subprocess test: `test_run_evolution_cli_fitness_learned_performance_requires_hyperparam_schema` — invoke `--fitness learned_performance` against `configs/evolution/mlpppo_foraging_small.yml` (which has no `hyperparam_schema`). Per task 5.3's ordering, guard (a) fires first regardless of `learn_episodes_per_eval` value, so no special fixture-config needed. Verify exit code is 1 and stderr names "hyperparam_schema" and "Lamarckian inheritance" / "M3" in the rationale
...
- [ ] 6.3 Pilot-config YAML comment header: explains the schema rationale, the cma_diagonal setting, and the decision-gate criteria. The cma_diagonal comment SHALL note that the explicit `cma_diagonal: false` matches `EvolutionConfig`'s field default at [config_loader.py:975](packages/quantum-nematode/quantumnematode/utils/config_loader.py#L975) — and the field-level comment at [config_loader.py:967-974](packages/quantum-nematode/quantumnematode/utils/config_loader.py#L967) explicitly says the default is aligned with M2's HyperparameterEncoder (n\<20 where full-cov is preferable). The pilot YAML setting it explicitly is documentation, not opt-out: writing the value in the pilot YAML makes the pilot self-documenting and visually symmetric with PR 3's LSTMPPO pilot YAML (which will set `cma_diagonal: true` for its larger genome). Future readers reading just the pilot YAML will see the choice without having to cross-reference the field default
...
- [ ] 10.5 Run `uv run pytest -m "not nightly"` — all green. M0+perf baseline is 2193; this PR adds ~41 new unit/subprocess tests (Phases 1-7 + 6 in Phase 4.5 [4.5.3-4.5.6 + 4.5.8 sibling pair] + 1 sibling test in 4.12 + 2 sibling tests in 4.10b/4.10c + 2 sibling tests in 5.4a) so the new total is ≈ 2234
