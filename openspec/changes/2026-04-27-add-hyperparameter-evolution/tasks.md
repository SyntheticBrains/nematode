...
- [ ] 6.3 Pilot-config YAML comment header: explains the schema rationale, the cma_diagonal opt-out, and the decision-gate criteria. The cma_diagonal comment SHALL note that although the YAML's explicit `cma_diagonal: false` matches `EvolutionConfig`'s field default at [config_loader.py:975](packages/quantum-nematode/quantumnematode/utils/config_loader.py#L975), the explicit setting is intentional documentation: this pilot has a *reason* for full-cov mode (n=7 is well within tractability) — distinct from PR 3's LSTMPPO pilot which will set `cma_diagonal: true` for its larger genome. Future readers should know the choice is deliberate, not inherited from the default
...
- [ ] 10.5 Run `uv run pytest -m "not nightly"` — all green. M0+perf baseline is 2193; this PR adds ~39 new unit/subprocess tests (Phases 1-7 + 6 in Phase 4.5 [4.5.3-4.5.6 + 4.5.8 sibling pair] + 1 sibling test in 4.12 + 2 sibling tests in 4.10b/4.10c) so the new total is ≈ 2232
