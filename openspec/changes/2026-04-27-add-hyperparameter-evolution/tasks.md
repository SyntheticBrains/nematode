...
- [ ] 4.7 Resolve eval-episode count: `L = evolution_config.eval_episodes_per_eval if evolution_config.eval_episodes_per_eval is not None else evolution_config.episodes_per_eval`. Falls back to `episodes_per_eval` so M0-shaped configs keep working
...
**Dependencies**: Phase 1 (needs `hyperparam_schema` field on `SimulationConfig`)
...
- [ ] 4.5.5 Unit test: `test_lineage_genome_carries_param_schema` — after a 1-gen run with a populated `hyperparam_schema`, read the lineage CSV does not need to contain the schema (lineage rows are flat — see M0's lineage format), but the in-memory `Genome` instance recorded by `LineageTracker.record(...)` SHALL have populated `birth_metadata`. Use a recording lineage tracker mock to capture the `Genome` arg
...
- [ ] 10.5 Run `uv run pytest -m "not nightly"` — all green. M0+perf baseline is 2193; this PR adds ~34 new unit/subprocess tests (Phases 1-7 + the 3 in Phase 4.5) so the new total is ≈ 2227
