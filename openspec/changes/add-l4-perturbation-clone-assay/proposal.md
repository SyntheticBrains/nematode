...
It is also the variant's **first contact with the connectome**. Everything so far has been the MLP
yardstick on a synthetic task; the clone arm runs the real substrate, the real environment and a
policy that is already competent.

## What Changes

- **One arm config**: the wild-type plastic clone arm with the variant's rule keys and nothing
  else changed — `plasticity_eligibility: node_perturbation` at the σ the control pinned.
...
- The runs, records under `supporting/050-l4-perturbation-clone-assay/`, tests, docs.
