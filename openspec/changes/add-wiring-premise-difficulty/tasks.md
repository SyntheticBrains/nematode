...
- [x] 1.1 Four configs from the committed food-only `_t20` base at **each calibration grid point**
...
- [x] 1.2 Extend the exact-key test to this family: each config differs from its base by the budget
  keys plus its own arm key and nothing else, with `rewire_seed` unset.

## 2. Harness

- [x] 2.1 Add the cell to `scripts/analysis/wiring_premise.py` — its `SCORED` metric (full-clear
...
- [x] 2.2 Confirm the family correction is per campaign: a manifest carrying only this cell corrects
...
- [x] 2.3 Tests for the new cell's rows, its verdict order, and the reproduction in 2.2.

## 3. Pilot (disjoint seeds 101–104)

- [x] 3.1 `launch.md` committed before the pilot runs.
- [x] 3.2 Per grid point and per pilot seed: does the cell learn, how far are the PPO arms from the
...
- [x] 3.3 Measured per-run wall time at the chosen budget, and the campaign scheduled from it.
- [x] 3.4 One budget frozen and recorded with the grid that produced it. If no grid point lands inside
