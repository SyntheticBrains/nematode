...
- [ ] 2.1 Running modulator scale `σ`: bias-corrected EMA, pre-update use, floor;
  `δ̃ = tanh(δ / σ)` when enabled; raw `δ` still reported.
- [ ] 2.2 Per-tensor running trace scale `ρ` over the masked entries: bias-corrected EMA over
...
- [ ] 2.5 Group the scaling options in a small dataclass handed to the rule, so the constructor
...
- [ ] 3.2 Modulator: bounded in `[−1, 1]`; equals `tanh(δ / σ)` with the pre-update `σ`;
...
- [ ] 3.7 Config: a scale rate outside `(0, 1]` or a non-positive floor fails at load; the
