...
- [x] 1.4 Implement `add_source(source: PheromoneSource)`

  - Append to `_sources`
  - If len exceeds `max_sources`, remove oldest sources

- [x] 1.5 Implement `get_concentration(position, current_step) -> float`
...
- [x] 1.6 Implement `get_gradient(position, current_step) -> tuple[float, float]`

  - Central differences: `(C(x+1,y) - C(x-1,y)) / 2, (C(x,y+1) - C(x,y-1)) / 2`
  - Return `(dx, dy)` vector
  - Pattern: same as OxygenField.get_gradient()

- [x] 1.7 Implement `get_gradient_polar(position, current_step) -> tuple[float, float]`
...
- [x] 1.8 Implement `prune(current_step)`

  - Remove sources where `current_step - emission_step > max_age`

- [x] 1.9 Add comprehensive tests in `tests/.../env/test_pheromone.py`
...
- [x] 2.3 Add `pheromones: PheromoneParams` to `DynamicForagingEnvironment.__init__()` and create PheromoneField instances when enabled
...
- [x] 2.4 Add concentration and gradient methods
...
- [x] 2.7 Add `update_pheromone_fields(current_step)` method

  - Prune expired sources on both fields
  - Called once per step from orchestrator

- [x] 2.8 Export `PheromoneField`, `PheromoneType`, `PheromoneSource`, `PheromoneParams` from `env/__init__.py`
...
- [x] 3.1 Add 8 pheromone fields to `BrainParams` in `brain/arch/_brain.py`
...
- [x] 3.2 Update `_create_brain_params()` in `agent/agent.py` to populate pheromone fields
...
- [x] 4.1 Add `PHEROMONE_FOOD` and `PHEROMONE_ALARM` to `ModuleName` enum

- [x] 4.2 Add `PHEROMONE_FOOD_TEMPORAL` and `PHEROMONE_ALARM_TEMPORAL` to `ModuleName` enum

- [x] 4.3 Implement `_pheromone_food_core(params) -> CoreFeatures`

  - Oracle: strength = gradient_strength, angle = relative angle from agent heading
  - Pattern: identical to `_food_chemotaxis_core()`

- [x] 4.4 Implement `_pheromone_alarm_core(params) -> CoreFeatures`

  - Oracle: strength = gradient_strength, angle = relative angle from agent heading
  - Pattern: identical to `_nociception_core()`

- [x] 4.5 Implement `_pheromone_food_temporal_core(params) -> CoreFeatures`

  - Temporal: strength = concentration [0,1], angle = tanh(dC/dt * derivative_scale)
  - Pattern: identical to `_food_chemotaxis_temporal_core()`

- [x] 4.6 Implement `_pheromone_alarm_temporal_core(params) -> CoreFeatures`

  - Temporal: strength = concentration [0,1], angle = tanh(dC/dt * derivative_scale)
  - Pattern: identical to `_nociception_temporal_core()`

- [x] 4.7 Register all 4 modules in `SENSORY_MODULES` dict

  - Each with appropriate `classical_dim=2`, descriptions referencing C. elegans ASK/ADL neurons

- [x] 4.8 Update `apply_sensing_mode()` in `config_loader.py`
...
- [x] 4.9 Add tests in `tests/.../brain/test_pheromone_modules.py`
...
- [x] 5.1 Make `STAMBuffer.num_channels` configurable (4 or 6)
...
- [x] 5.2 Update channel index constants
...
- [x] 5.3 Update `record()` to accept variable-length scalars

  - `scalars: np.ndarray` shape (num_channels,) instead of fixed (4,)
  - Validate length matches num_channels

- [x] 5.4 Update `get_memory_state()` for dynamic dimensions
...
- [x] 5.5 Update `STAMSensoryModule` in `brain/modules.py`
...
- [x] 5.6 Update `_compute_temporal_data()` in `agent/agent.py`
...
- [x] 5.7 Add STAM extension tests
...
- [x] 6.1 Add step counter to `run_episode()` for pheromone aging
...
- [x] 6.2 Emit food-marking pheromone after food consumption in `_resolve_food_step()`

  - For each food winner: `env.emit_food_pheromone(consumed_position, current_step, agent_id)`
  - Only when pheromones enabled

- [x] 6.3 Emit alarm pheromone after predator damage
...
- [x] 7.1 Create `PheromoneTypeConfig` and `PheromoneConfig` Pydantic models in `config_loader.py`
...
- [x] 7.2 Add `pheromones: PheromoneConfig | None = None` to `EnvironmentConfig`

- [x] 7.3 Add `pheromone_food_mode: SensingMode` and `pheromone_alarm_mode: SensingMode` to `SensingConfig`

  - Default: `SensingMode.ORACLE`

- [x] 7.4 ~~Update `apply_sensing_mode()` for pheromone modules~~ (covered by task 4.8)

- [x] 7.5 Update `create_env_from_config()` to pass pheromone params to environment

- [x] 7.6 Add config validation tests
...
- [x] 8.1 Add `agent_id` to `_SIMULATION_RESULTS_FIELDNAMES` in `report/csv_export.py`

  - Insert as second field (after "run")
  - Update `_simulation_result_to_row()` to include agent_id

- [x] 8.2 Write per-agent CSV rows in `_run_multi_agent()`
...
- [x] 10.1 Create `configs/scenarios/multi_agent_foraging/mlpppo_medium_2agents_pheromone_oracle.yml`
...
- [x] 10.2 Create `configs/scenarios/multi_agent_pursuit/mlpppo_medium_5agents_pheromone_oracle.yml`
...
- [x] 10.3 Create `configs/scenarios/multi_agent_foraging/lstmppo_medium_2agents_pheromone_temporal.yml`
...
- [x] 10.4 Create `configs/scenarios/multi_agent_pursuit/lstmppo_medium_2agents_pheromone_temporal.yml`
...
- [x] 11.1 Run `uv run pytest -m "not nightly"` — ALL tests pass

- [x] 11.2 Run `uv run pre-commit run -a` — all hooks pass

- [x] 11.3 Backward compatibility: existing multi-agent configs without pheromones produce identical behavior

- [x] 11.4 Sanity check: 2-agent with food-marking pheromones, verify pheromone sources created on food consumption

- [x] 11.5 Sanity check: multi-agent with alarm pheromones + predators, verify alarm sources created on predator damage

- [x] 11.6 Update `docs/roadmap.md`

  - Update Phase 4 status with pheromone communication progress

- [x] 11.7 Update `AGENTS.md`
...
- [x] 11.8 Update `openspec/config.yaml`

  - Mention pheromone communication in Architecture section

- [x] 11.9 Update `configs/README.md`
