...
- [ ] 1.2 Validators: half-specified schedules, a final scale above the initial one and a zero
...
- [ ] 2.1 A single scheduling helper the two topologies share, so the shape exists once: steps
  begun in, current scale out, with the geometric path and the floor after `E`.
- [ ] 2.2 A plain-integer counter on each topology and an `advance_schedule()` seam method, added
...
- [ ] 2.3 Both brains' `prepare_episode` call `advance_schedule()` beside `reset_traces()`; the
...
- [ ] 2.4 Tests: the scale falls monotonically and hits the floor exactly at `E`; resetting the
...
- [ ] 3.1 An annealed arm in `scripts/analysis/l4_rule_positive_control.py` at the registered
...
- [ ] 3.2 The harness reports gradient alignment over the decay and over the floor separately,
...
- [ ] 4.1 Two arm configs at the registered values (0.2 → 0.02 over the first 1,000 of 2,000
...
- [ ] 4.3 The harness bins both arms' curves per 250 episodes with σ(e) stated per bin, so the
