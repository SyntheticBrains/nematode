...
**Status**: PR 3 in flight — both pilot arms GO under the bug-fixed framework (MLPPPO +5.5pp, LSTMPPO+klinotaxis +7.5pp), but both saturate at 1.000 from gen 1 so the GO decisions are mechanically clean but scientifically uninformative. M2.11 (LSTMPPO + klinotaxis + pursuit predators) opens a non-saturated fitness landscape in the follow-up PR before M3 starts. Roadmap row holds at 🟡 in progress until M2.11 lands
**Bio fidelity**: LOW
...
- [ ] M2.11 Add a third M2 arm — **LSTMPPO + klinotaxis + pursuit predators** — to produce a non-saturated fitness landscape (next PR after PR 3 merges, before M3). Both current arms (MLPPPO+oracle, LSTMPPO+klinotaxis foraging) saturate at 1.000 in essentially every generation, making the pilot's GO decision vacuous and carrying that into M3's Lamarckian inheritance pilot if not addressed. Predator pressure forces the brain to balance foraging and survival — the classic regime where gamma/entropy/lr differentiate policies. Same brain + sensing + scripts pattern as the existing LSTMPPO arm; adds predator env config and a re-run. After this lands, decide whether to also add multi-modality arms (thermotaxis/aerotaxis per logbook 010) or move directly to M3
