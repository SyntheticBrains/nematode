...
**Status**: complete — all three arms GO. MLPPPO+oracle +5.5pp (saturated), LSTMPPO+klinotaxis foraging +7.5pp (saturated), LSTMPPO+klinotaxis+predator (M2.11) **+47.0pp on 4 seeds, non-saturated** with 3/4 reaching 0.92 best fitness via real CMA-ES climbing and 1/4 dead-zone failure. The predator arm is the M3 prerequisite — non-saturated landscape, reproducible-with-noise, CMA-ES-trainable, with a known dead-zone failure mode that M3's inheritance must improve over
**Bio fidelity**: LOW
