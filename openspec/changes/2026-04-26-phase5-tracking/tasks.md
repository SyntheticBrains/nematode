...
**Status**: PR 4 (M2.11) in flight; one open follow-up (M2.12 / RQ1 close-out) before M3. Three arms GO so far: MLPPPO+oracle +5.5pp (saturated), LSTMPPO+klinotaxis foraging +7.5pp (saturated), LSTMPPO+klinotaxis+predator (M2.11) **+47.0pp on 4 seeds, non-saturated** with 3/4 reaching 0.92 best fitness via real CMA-ES climbing and 1/4 dead-zone failure. The predator arm is the M3 prerequisite. M2.12 closes RQ1 (Optuna/TPE on the predator config) before M3 starts — the dead-zone failure surfaced in M2.11 is exactly the kind of pathology TPE handles better than CMA-ES. Roadmap row holds at 🟡 in progress until M2.12 lands
**Bio fidelity**: LOW
...
- [ ] M2.12 Close RQ1: add an Optuna/TPE optimiser adapter and re-run the M2.11 predator-arm pilot under it (next PR before M3). Same brain + sensing + 6-field schema + K/L budget + 4 seeds — only the optimiser changes from CMA-ES to Optuna's TPE sampler. The seed-43 dead-zone failure in M2.11 (CMA-ES converged on actor_lr=1e-5 + entropy≈0 region) is the canonical pathology TPE's tree-structured prior + early pruning are designed to handle. Decision per RQ1 escalation rule: if TPE's mean final fitness across seeds is ≥5pp higher than CMA-ES's (and/or rescues seed 43 specifically), open a follow-up change `<DATE>-add-optuna-optimizer` to make Optuna a first-class optimiser; else close RQ1, document CMA-ES stays the default for M3+. Bounded scope: one optimiser adapter + one re-run + one logbook addendum to 012. Estimated wall: ~half day code + ~40 min re-run. M3 starts directly on whichever optimiser wins.
...
**Status**: triggered — to be closed in M2.12 (next PR)
**Trigger**: M2.11 (predator arm) surfaced a CMA-ES dead-zone failure mode (seed 43 stuck at 0.000 across 20 generations because CMA-ES converged on `actor_lr` clipped at lower bound + `entropy_coef` ≈ 0). 1/4 dead-trajectory rate is exactly the kind of pathology TPE's tree-structured prior + pruning are designed to avoid.
**Bounded follow-up scope**: M2.12 — one OpenSpec change that adds one optimiser adapter (Optuna's TPE sampler) alongside the existing CMA-ES + GA, with a comparison run on the **M2.11 predator pilot config** (the only non-saturated arm, so the only one where the comparison is meaningful)
