...
- [x] M5.5 Run 50+ gen × 4 seed campaign — **superseded by PR 6's screening pivot** (NOT executed; deferred). The originally planned 2-seed × 30-gen pilot expanded into a 10-screen lever sweep when the canonical pilot's verdict gate produced ambiguous signal under the K-block-elite series. No candidate produced strict Red Queen entanglement (own-vs-cross lag delta stuck at +0.017 to +0.024 across all screens, including the GA optimiser fallback design.md D2 reserved for this regime). **Pilot recommendation: STOP M5.** Full 50+ gen × 4 seed run not executed; the screening evidence is decisive (saving ~30 wall-hours). See [logbook 017](../../../docs/experiments/logbooks/017-coevolution-arms-race.md) for the screening trajectory + STOP verdict rationale + methodology contributions.
...
- [x] M5.8 Publish `artifacts/logbooks/017/` with Red Queen findings — done. [`docs/experiments/logbooks/017-coevolution-arms-race.md`](../../../docs/experiments/logbooks/017-coevolution-arms-race.md) covers the screen sweep, lag-matrix methodology, fair-test methodology, STOP M5 verdict + rationale, and M6.5 NEAT as the natural Red Queen follow-up. Artefact stash under [`artifacts/logbooks/017-coevolution-arms-race/`](../../../artifacts/logbooks/017-coevolution-arms-race/) — per-screen post-hoc summaries + champion archives + summary CSVs (fair-test, verdict, lag matrix) + c2 analysis scripts.
