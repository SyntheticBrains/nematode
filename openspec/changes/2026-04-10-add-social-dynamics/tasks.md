...
- [x] 1.1 Add `SocialFeedingParams` dataclass to `env/env.py`

  - Fields: `enabled: bool = False`, `decay_reduction: float = 0.7`, `solitary_decay: float = 1.0`

- [x] 1.2 Wire `SocialFeedingParams` into `DynamicForagingEnvironment.__init__()` and `copy()`

  - `self.social_feeding: SocialFeedingParams` attribute
  - Pass through in `copy()` (same pattern as PheromoneParams)

- [x] 1.3 Add `decay_multiplier` parameter to `SatietyManager.decay_satiety()`
...
- [x] 1.4 Add `SocialFeedingConfig` to `config_loader.py`
...
- [x] 1.5 Add `social_phenotype` to `AgentConfig` in `config_loader.py`

  - `social_phenotype: Literal["social", "solitary"] = "social"`

- [x] 1.6 Apply social feeding decay reduction in `multi_agent.py`
...
- [x] 1.7 Track `social_feeding_events` counter in MultiAgentSimulation

  - Increment when decay reduction is applied (multiplier != 1.0 and nearby > 0)

- [x] 1.8 Export `SocialFeedingParams` from `env/__init__.py`

- [x] 1.9 Unit tests for social feeding
...
- [x] 2.1 Add `AGGREGATION = "aggregation"` to `PheromoneType` enum in `env/pheromone.py`

- [x] 2.2 Add `aggregation: PheromoneTypeConfig` to `PheromoneParams` in `env/env.py`

  - Defaults: `emission_strength=0.5, spatial_decay_constant=10.0, temporal_half_life=10.0, max_sources=200`

- [x] 2.3 Create `pheromone_field_aggregation: PheromoneField | None` in `DynamicForagingEnvironment.__init__()`

  - Created when pheromones enabled and aggregation config present
  - Update `copy()` to include aggregation field

- [x] 2.4 Add aggregation pheromone methods to environment
...
- [x] 2.5 Update `update_pheromone_fields()` to prune aggregation field

- [x] 2.6 Add continuous aggregation emission in `multi_agent.py` step loop
...
- [x] 2.7 Add 4 BrainParams fields for aggregation sensing in `brain/arch/_brain.py`
...
- [x] 2.8 Add `PHEROMONE_AGGREGATION` and `PHEROMONE_AGGREGATION_TEMPORAL` sensing modules in `brain/modules.py`
...
- [x] 2.9 Populate aggregation pheromone fields in `agent/agent.py`
...
- [x] 2.10 Extend STAM for 7-channel mode in `agent/stam.py`
...
- [x] 2.11 Update `config_loader.py` for aggregation pheromone
...
- [x] 2.12 Unit tests for aggregation pheromone
...
- [x] 3.1 Extend `MultiAgentEpisodeResult` with 4 new metric fields
...
- [x] 3.2 Implement `aggregation_index` computation
...
- [x] 3.3 Implement `alarm_evasion_events` tracking
...
- [x] 3.4 Implement `food_sharing_events` tracking
...
- [x] 3.5 Wire metrics into `_build_result()` in multi_agent.py

- [x] 3.6 Update `csv_export.py` with new summary columns

  - Add `social_feeding_events`, `aggregation_index`, `alarm_evasion_events`, `food_sharing_events` to multi_agent_summary fieldnames

- [x] 3.7 Unit tests for collective metrics
...
- [x] 4.1 Create `configs/scenarios/multi_agent_foraging/mlpppo_medium_5agents_social_oracle.yml`

  - 5 agents, social feeding enabled, no aggregation pheromone
  - Tests social feeding in isolation

- [x] 4.2 Create `configs/scenarios/multi_agent_foraging/mlpppo_medium_5agents_aggregation_oracle.yml`

  - 5 agents, aggregation pheromone enabled, no social feeding
  - Tests aggregation pheromone in isolation

- [x] 4.3 Create `configs/scenarios/multi_agent_foraging/mlpppo_medium_5agents_full_social_oracle.yml`

  - 5 agents, social feeding + all pheromones (food-marking, alarm, aggregation)
  - Full social dynamics stack

- [x] 4.4 Run `uv run pytest -m "not nightly"` — all tests pass

- [x] 4.5 Run `uv run pre-commit run -a` — all hooks pass

- [x] 4.6 Update `docs/roadmap.md` Phase 4 status

- [x] 4.7 Update `AGENTS.md` with social dynamics info

- [x] 4.8 Update `openspec/config.yaml` Architecture section

- [x] 4.9 Update `configs/README.md` with new scenario configs
