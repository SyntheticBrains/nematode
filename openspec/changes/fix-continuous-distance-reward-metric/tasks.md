...
- [ ] 3.2 Add a continuous-substrate test asserting the foraging distance term telescopes (tangential
...
- [ ] 4.1 Re-run C1 foraging convergence n≥4 across MLP / Transformer / LSTM / CfC with the corrected
...
- [ ] 4.2 Record per-arch converged foraging (final-quarter completed-target success + efficiency) in the
      T7 forensics scratchpad → T7 logbook; note the corrected metric and the spurious-reward removal.
- [ ] 4.3 Re-assess `add-ppo-value-normalization`: with the corrected reward, determine whether value
...
- [ ] 5.1 `openspec validate fix-continuous-distance-reward-metric --strict`.
- [ ] 5.2 Targeted `pre-commit` (ruff / pyright / markdownlint) on changed files; reward-calculator +
      continuous-env test suites pass (incl. grid byte-stability).
- [ ] 5.3 Note in `phase6-tracking` that the T7 C1 recurrent collapse root cause was the continuous
