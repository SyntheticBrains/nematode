...
- [ ] A.4 Baseline arms pinned per D2: frozen-weights + vanilla-rule sanity floors. **Two findings inherited from A.3's review, both to resolve here**: (a) D2 describes the frozen-weights init as "Cook-2019 synapse-count-derived", but the implementation uses the connectome for *edge existence* only — weights are drawn `N(0, 1/sqrt(chemical in-degree))` and `syn.weight` never reaches `w_chem`; either correct the wording or change the initialisation deliberately. (b) The connectome brain implements **no weight-component persistence at all**, which a frozen-weights arm needs. This arm is also the detector for A.3's accepted risk: a plastic arm that fails to clear this floor reports "the rule did not learn", never "the wiring is inert".
