...
- [ ] 1.1 Add a source-depletion block to `ForagingParams` (`env/env.py:~231`) with sensible defaults: `source_depletion_enabled: bool = False`, `source_initial_amount: float = 1.0`, `depletion_per_feed: float = 1.0`, `removal_eps: float = 1e-3` (and an optional `deplete_scales_reward: bool = False` lever, D7).
...
- [ ] 7.3 Run the same panel + seeds on the `no_respawn`-only control cell (6.2).
