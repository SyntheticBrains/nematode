...
- [ ] 7.0 Curate the prey held-out bundle: sample `held_out_size` (default 8) random elite genomes across all 4 M3 lamarckian-LSTMPPO full-run seeds (e.g. 2 from each seed) from `artifacts/logbooks/<M3-lamarckian-logbook>/`; copy the JSONs to `configs/evolution/coevolution_held_out_prey/`; write a `README.md` next to them documenting provenance (source run, seed, generation, fitness, brain config). Required for task 6.7 to load.
...
- **PR 3** (HoF + Red Queen metrics + CoevolutionLoop): tasks 4.1–4.4 + 5.1–5.7 + 6.1–6.10. ~1100 LoC + ~480 LoC tests (CoevolutionLoop carries ~100 LoC of replicated inheritance bookkeeping per design D10). Largest PR; consider further splitting if review feedback suggests.
- **PR 4** (Configs + held-out bundle + driver + smoke): tasks 7.0–7.6. ~500 LoC + held-out bundle (~80-200 KB JSONs); smoke validates the substrate before pilot.
