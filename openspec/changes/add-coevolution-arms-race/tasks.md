...
- [ ] 1.1 Create `quantumnematode/env/mlpppo_predator_brain.py` with `MLPPPOPredatorBrain` implementing the `PredatorBrain` Protocol; mirror agent-side MLPPPO via composition (`actor_hidden_dim=64`, 2 hidden layers, value head); implement `WeightPersistence` for encoder round-trip.
...
- [ ] 2.3 Add `mlpppo_predator` branch to `_build_predator_brain` in `quantumnematode/env/env.py:1538` (~6 lines) constructing `MLPPPOPredatorBrain` from `extra` config; optionally accept a weight-load path so the brain can be initialised from a saved genome's checkpoint.
...
- [ ] 3.3 Create `quantumnematode/evolution/predator_fitness.py` with `PredatorEpisodicKillRate` (primary kill-rate signal) and `PredatorLearnedPerformanceFitness` (learned-PPO secondary variant); both implement the existing `FitnessFunction` Protocol.
- [ ] 3.4 Implement secondary proximity signal: when kill_count is zero across all results, fall back to `mean(per_predator_prey_proximity_steps) / max_steps`; ensure the secondary signal is strictly less than the smallest kill-rate fitness.
...
- [ ] 3.6 Add `tests/quantumnematode_tests/evolution/test_predator_fitness.py` (~8 cases): kill-rate evaluation against synthetic `MultiAgentEpisodeResult` with mock kills; secondary proximity signal triggered when kills=0; FitnessFunction Protocol conformance via `isinstance`.
...
- [ ] 6.3 Implement per-K-block TPE study reset: at each K-block transition, the just-flipped side's optimizer's TPE study is cleared and re-seeded.
...
- [ ] 6.7 Implement held-out opponent construction: prey side draws from external champions (configurable path to M3 Lamarckian baseline elites); predator side draws from heuristic-radius variants spanning configured `detection_radius` × `damage_radius` grid.
...
- [ ] 7.3 Create `scripts/campaigns/run_coevolution.py`: per-seed driver invoking `CoevolutionLoop.run`; argparse + multiprocessing + JSON+CSV outputs; mirror `scripts/campaigns/phase5_m3_lamarckian_lstmppo_klinotaxis_predator.sh` template.
...
- [ ] 10.1 Lock full-run thresholds into the OpenSpec change `co-evolution` capability spec (cycling lag-range, FFT power threshold, escalation slope/SE threshold, gen-window) before launching full run.
...
- [ ] 11.4 Append M5.7 readout section to `docs/experiments/logbooks/017-coevolution-arms-race.md`: instrumentation results clearly labelled "secondary observation, not a verdict input"; if both readout conditions fire → flag M4.7 as triggered; if not → flag as observation-only outcome.
