...
- **AND** the predator side state SHALL be configured with `PredatorEpisodicKillRate`; predator inheritance SHALL be YAML-driven via `predator_evolution.inheritance ∈ {"none", "lamarckian"}` with default `"none"` (frozen-weight evaluation, N_eval episodes, NO inner-loop PPO training, NO weight capture — the design.md D13 default). Opting in to `"lamarckian"` enables predator-side PPO inner-loop training under the multi-agent runner's per-step `predator.brain.learn(reward, episode_done)` hook and requires `predator_evolution.learn_episodes_per_eval > 0` plus a predator brain built with `enable_learning=True` (the `MLPPPOPredatorBrain` path constructs the optimizer + rollout buffer in this mode). The `PredatorEpisodicKillRate.evaluate` kwargs `warm_start_path_override` and `weight_capture_path` mirror `LearnedPerformanceFitness.evaluate`'s surface when Lamarckian is opted in
- **AND** the default asymmetric fitness shape is intentional per design.md D13 — prey trains per evaluation (large policy space, M3 substrate); predator evaluates frozen (small policy space, direct CMA-ES weight gradient suffices). The `"lamarckian"` opt-in exists as a substrate ablation for runs that want symmetric inheritance plumbing; M5 pilot screening exercised the opt-in path without producing strict Red Queen entanglement
