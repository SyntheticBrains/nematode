...
- [x] 1.1 Extend the in-repo spiking substrate (`brain/arch/_spiking_layers.py` or a sibling module):
...
- [x] 1.2 Add `SPIKING_PPO` to the `BrainType` enum in `brain/arch/dtypes.py` (value `"spikingppo"`) and
  to the `BRAIN_TYPES` `Literal`. (`families=("spiking",)` auto-populates `SPIKING_BRAIN_TYPES` via
  the registry — no manual set edit.)

## 2. Config

- [x] 2.1 Add `SpikingPPOBrainConfig(BrainConfig)` with: `sensory_modules` (validated non-empty);
  ...
  base fields.
- [x] 2.2 `@model_validator(mode="after")` checks: `sensory_modules` non-empty; positive/range guards on
...
- [x] 3.1 `SpikingPPOBrain(ClassicalBrain)` with
  `@register_brain(name="spikingppo", config_cls=SpikingPPOBrainConfig, brain_type=BrainType.SPIKING_PPO, families=("spiking",))`.
- [x] 3.2 Networks: `feature_norm = nn.LayerNorm(input_dim)`; learnable direct-current encoder
...
- [x] 3.3 Single carried neuron state `(v, a, s, m)` zeroed in `prepare_episode()`, carried across steps,
  one LIF update per env-step (or `timesteps_per_step` inner ticks with a constant input current).
- [x] 3.4 `preprocess` (reuse `extract_classical_features`), `run_brain` (feature_norm → encode current →
...
- [x] 3.5 `SpikingPPORolloutBuffer` + `_perform_ppo_update` mirroring `CfCPPOBrain` (GAE; truncated-BPTT
...
- [x] 3.6 `_get_entropy_coef()` (reuse the CfC linear-anneal schedule) and `_get_surrogate_slope()`
  ...
  BPTT replay.
- [x] 3.7 `WeightPersistence`: `get_weight_components` / `load_weight_components` for the input encoder,
...
- [x] 3.8 No milestone/tracking labels in code or docstrings.

## 4. Registry integration

- [x] 4.1 Import the new module in `brain/arch/__init__.py` (and add the class + config to `__all__`) so
  the decorator runs (match how `cfcppo` / `lstmppo` are imported).
- [x] 4.2 Add `SpikingPPOBrainConfig` to the `BrainConfigType` union in `utils/config_loader.py` (+ its
...
- [x] 5.2 Forward-pass shapes: logits `(num_actions,)`, finite; critic value scalar, finite; output dim
  equals `num_actions`.
- [x] 5.3 Recurrent neuron state carries within an episode + resets on `prepare_episode()`; two identical
  inputs at different in-episode steps can differ (carried state) — i.e. genuinely recurrent.
- [x] 5.4 A PPO update runs without error on a small synthetic rollout, with gradient flowing through the
  surrogate, leaving params finite.
- [x] 5.5 Weight `get`/`load` round-trip → identical logits for the same input + neuron state.
- [x] 5.6 Determinism under a fixed seed.
- [x] 5.7 Substrate-level tests for the recurrent adaptive-LIF cell (decay, soft reset, adaptation,
...
- [x] 5.8 Schedule validators: `entropy_coef_end` / `entropy_decay_episodes` and `surrogate_slope_end` /
  `surrogate_slope_anneal_episodes` each reject a half-set pair with a clear error; flat when unset;
  anneal-then-hold when set.
- [x] 5.9 Non-degenerate variance: over a sample of ≥ 100 forward passes with non-zero gradients, the
...
- [x] 6.1 Create `configs/scenarios/foraging/spikingppo_small_klinotaxis.yml` mirroring
  `configs/scenarios/foraging/cfcppo_small_klinotaxis.yml` (identical env/reward/sensing; brain block
  `spikingppo`).
- [x] 6.2 `uv run pytest -m "not nightly and not slow" tests/quantumnematode_tests/brain/arch/test_spikingppo.py` — green.
...
- [x] 7.1 `openspec validate add-spiking-ppo-brain --strict` clean.
- [x] 7.2 Add `spikingppo` to the brain enumeration + bump the count to **23** in BOTH `AGENTS.md` (the
  `brain/arch/` list) and `openspec/config.yaml` (the architecture list).

## Implementation notes (deviations from the as-drafted spec)

- **`adaptation_decay` (ρ) is a fixed decay, not learnable** — the design left this "learnable or fixed";
  the adaptive-threshold gain is learned via `adapt_scale` (per-neuron `nn.Parameter`), while ρ stays a
  config-set constant (`adaptation_decay_init`). Matches Decision 1's load-bearing set (learnable decay +
  adaptive threshold + recurrent feedback).
- **`RecurrentAdaptiveLIFCell.input_dim` is stored but unused in the forward** — the encoder upstream
  already projects to `hidden_size`, so the cell operates on `num_neurons`-wide currents; `input_dim` is
  kept only for symmetry with `LIFLayer` (the brain passes `input_dim == hidden_size`). Harmless.
- **Logits = the final readout membrane** after the inner-tick loop (the `LeakyIntegratorReadout` output
  IS its membrane), rather than a separate logits variable.
- **Substrate cells added to `_spiking_layers.py`** (`RecurrentAdaptiveLIFCell`, `RecurrentAdaptiveLIFState`,
  `LeakyIntegratorReadout`, an `_inverse_sigmoid` helper) rather than a sibling module;
  `SurrogateGradientSpike` and `LIFLayer` are reused unchanged.
- **`_validate_config` carries `# noqa: C901, PLR0912`** (more range guards than CfC); the actor
  parameter list is factored into `_actor_parameters()`.
- **Foraging smoke (seed 42, 300 ep): full-clear success climbs 4% → 66% across the run** (per-50-block:
  4 / 20 / 12 / 40 / 46 / 66%), 6.18/10 foods avg — learns well above random, no dead-exploration
  collapse. The combined-cell C3 de-risk + n-seed sweep are downstream (the comparison), not this change.
