...
- **New `WeightPersistence` protocol** — A separate `@runtime_checkable` protocol (not added to `Brain`) with component-based save/load. Brains opt in by implementing `get_weight_components()` and `load_weight_components()`. Brains that don't implement it fall back to the existing `plasticity/snapshot.py` generic mechanism.
...
- **Hybrid brain wrappers** — `HybridQuantumBrain` and `HybridClassicalBrain` gain thin `WeightPersistence` wrappers delegating to existing private `_save/_load` methods. Existing `qsnn_weights_path`/`cortex_weights_path` config fields continue working.
...
- **Auto-save final weights** — All brains auto-save `final.pt` to `exports/{session_id}/weights/` at training end (no flag needed).
...
- `weight-persistence`: Core weight persistence protocol, types, save/load functions, component filtering, metadata validation, and generic snapshot fallback.
...
- **Code**: New module `brain/weights.py`. Edits to `brain/arch/dtypes.py` (BrainConfig), `brain/arch/mlpppo.py`, `brain/arch/hybridquantum.py`, `brain/arch/hybridclassical.py`, `scripts/run_simulation.py`.
- **Config**: All brain YAML configs gain optional `weights_path` field (backward compatible, defaults to null).
- **Dependencies**: No new dependencies — uses existing PyTorch `torch.save`/`torch.load` and `plasticity/snapshot.py`.
