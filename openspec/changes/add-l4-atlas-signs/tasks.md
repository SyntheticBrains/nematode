...
- [ ] 2.2 A generation script that writes the atlas's transmitter values into the checked-in
...
- [ ] 2.3 Tests: the committed table's transmitters equal what the atlas loader re-derives, entry
...
- [ ] 3.2 `enforce_synapse_signs: bool = False` on `ConnectomePPOBrainConfig` (not the shared
...
- [ ] 3.3 Tests: the default is bit-identical; under `atlas` magnitudes and per-neuron incoming
...
- [ ] 4.1 Six configs, each one or two keys off a committed parent, naming keeping the parent as a
