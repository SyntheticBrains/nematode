...
- [ ] 2.3 Verify the existing `select_parents` call site at `loop.py:526` already runs for any non-`NoInheritance` strategy via the existing `if inheritance_on:` guard — Baldwin's `select_parents` returning `[best_genome_id]` will populate `self._selected_parent_ids` correctly without further loop changes. The GC step at `loop.py:532-534` MUST be guarded on `kind() == "weights"` (not on `inheritance_on`) so Baldwin doesn't attempt to GC a non-existent `inheritance/` directory.
