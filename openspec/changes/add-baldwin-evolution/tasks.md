...
- [ ] 4.5 Persist `_gens_without_improvement` (int) and `_last_best_fitness` (float | None) in the checkpoint pickle. Bump `CHECKPOINT_VERSION` 2 → 3. Also: when early-stop fires, the loop SHALL save a final checkpoint before breaking out of the main loop (regardless of `checkpoint_every` schedule), so a subsequent `--resume` correctly observes the run as complete rather than rewinding to the last `checkpoint_every`-aligned save.
