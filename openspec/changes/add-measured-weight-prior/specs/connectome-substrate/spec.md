...
endpoints both lie in the table's neuron set — together with the table entries that fall on a gap
junction only or on no connection, which SHALL be reported and not applied.
...
- **THEN** the covered edges, the head-scope edges, the positive and negative counts, and the entries
  on a gap junction only or on no connection SHALL equal the figures the change records
