...
This change builds a proper continuous-substrate renderer: sub-cell worm motion on a full-arena plate view (with an optional agent-following camera), with the chemosensory fidelity (concentration heatmap, gradient field, sensor zones, adaptive-sensor state) overlaid — and the offline matplotlib figures (trajectory / field / quiver) for logbooks.

## What Changes

- **A `Continuous2DRenderer`** (sibling of the existing `PygameRenderer`) that renders the continuous-2D arena at sub-cell fidelity via a camera-aware `world→pixel` map with a configurable pixels-per-mm zoom. The default view is the **full-arena plate** (no scroll); a **keyboard toggle (`C`)** switches to an **agent-following camera** (zoomed, worm-centred, clamped to the plate edges) so the worm stays legible on large worlds. The worm renders as a point + heading line (continuous `heading_rad`).
...
- **Live keyboard controls** for the single-agent renderer (which had none): `H` heatmap on/off, `F` cycle heatmap field, `G` gradient quiver on/off, `C` full-arena/agent-following camera. The status bar word-wraps to the window width so its rows are never clipped on narrow (small-world) windows.
