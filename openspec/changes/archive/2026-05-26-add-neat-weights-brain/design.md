...
**No `parameter_initializer` plumbing.** `MLPPPOBrain.__init__` accepts a `parameter_initializer` kwarg passed by `_build_infra_kwargs` at [`utils/brain_factory.py:117-126`](../../../packages/quantum-nematode/quantumnematode/utils/brain_factory.py#L117-L126); the default-shape fallthrough at [`utils/brain_factory.py:155-160`](../../../packages/quantum-nematode/quantumnematode/utils/brain_factory.py#L155-L160) does NOT pass it. `FeedforwardGABrain` uses the default-shape path, so it does not receive a `parameter_initializer`. This is deliberate: the `GeneticAlgorithmOptimizer` constructs the initial population from `x0`-scaled Gaussian noise per `evolutionary.py:331-336` and overwrites the brain's weights at decode-time via `WeightPersistence.load_weight_components()` — any brain-side init done by `parameter_initializer` would be discarded immediately on the first decode. The brain's own default-init (e.g. `nn.Linear`'s built-in Kaiming-uniform) is therefore sufficient and never user-visible.
