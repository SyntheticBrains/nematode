...
- [x] 1.1 Add the consolidation selector and its parameters to the shared plasticity mixin with
  ...
  `(0, 1]`.
- [x] 1.2 Validator: a selector other than `none` that its own parameters leave inert is rejected
...
- [x] 1.3 Tests: each bound; the inert-mechanism rejection for `anchor` and `rigidity`; the
  default config is unchanged field for field from today's.

## 2. The elastic anchor

- [x] 2.1 Allocate one anchor per plastic tensor, initialised to the constructed weights, only
  when `anchor` is selected.
- [x] 2.2 Apply `− η · κ_a · (w − a)` inside the masked update beside the decay; advance the
  anchor after the write.
- [x] 2.3 Tests: the restoring term's sign and magnitude; an anchor rate of zero holds the anchor;
  a positive rate moves it by exactly the configured fraction; nothing is written off the edge set.

## 3. Reinforced rigidity

- [x] 3.1 Allocate one protective variable per plastic tensor, zero at construction, only when
  `rigidity` is selected.
- [x] 3.2 Divide the Hebbian term's rate by `1 + κ_c · c` using the pre-growth value; advance
...
- [x] 3.3 Tests: rigidity grows where a positive modulator meets a large trace and not where the
...
- [x] 4.1 Accept an episode-success flag on the rule and maintain the trailing rate; scale the
  plasticity rate by `clamp(1 − s / s_ref, 0, 1)`.
- [x] 4.2 Forward the flag from the brain's existing `post_process_episode(episode_success=...)`
  hook; the hook stays a no-op under every other selector.
- [x] 4.3 State in the implementation that this consumes the task's scoring rather than the
  reward stream, and is a diagnostic bound rather than a mechanism.
- [x] 4.4 Tests: the rate reaches zero at the reference and is unchanged far below it; the flag
  reaches the rule through the brain hook; no other selector reads it.

## 5. State lifecycle and telemetry

- [x] 5.1 `reset_state()` re-anchors the anchor to the current weights and zeroes the protective
  variable, beside the existing resets.
- [x] 5.2 Report the effective rate multiplier, the mean anchor departure and the mean protective
  variable; record them in the shared plasticity report.
- [x] 5.3 Tests: a loaded clone is anchored to itself and its first restoring term is zero; the
...
- [x] 6.1 Test: with the selector `none`, a fixed-seed run's weight trajectory is bit-identical to
  the same run before this change.
- [x] 6.2 Test: consolidation composes with grounded signs and homeostasis in the registered
  order, with the clamp last.
- [x] 6.3 Smoke-test entries for one consolidated config.

## 7. The screen

- [x] 7.1 Clone-arm configs for the three variants, deriving from the existing wild-type plastic
  clone config with the rule keys and nothing else changed.
- [x] 7.2 A screen harness reading the campaign directory and reporting, per variant, the eight
