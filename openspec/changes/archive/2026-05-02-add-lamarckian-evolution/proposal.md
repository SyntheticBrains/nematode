...
- **MODIFIED** `EvolutionConfig` gains `inheritance: Literal["none", "lamarckian"] = "none"` and `inheritance_elite_count: int = 1`. **Six** validators reject invalid combinations: no train phase, conflict with static `warm_start_path`, missing `hyperparam_schema`, architecture-changing schema fields, `elite_count > population_size`, and the M3-only restriction `inheritance != "none"` AND `elite_count != 1` (multi-elite is M4-or-later — the field accepts >1 in the schema so M4 can lift this rule cleanly). The CLI also gains a startup guard rejecting `inheritance != "none" + --fitness success_rate` since the EvolutionConfig validator can't see the `--fitness` flag.
