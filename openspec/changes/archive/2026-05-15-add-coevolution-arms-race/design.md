...
**Stakeholders:** chris (Phase 5 lead, decision authority on verdict gates and compute budget). The OpenSpec change `add-coevolution-arms-race` (this change) is the spec contract; `openspec/changes/phase5-tracking/tasks.md` (M5.0–M5.9) is the milestone tracker that this change ticks.
...
- **Same provenance as held-out bundle (and same files).** Task 7.0a/b curates `configs/evolution/coevolution_warmstart_prey/` from M3 lamarckian elites. The gen-0 anchor and the held-out probe set read from the same directory — one bundle, two roles, same provenance file format.
