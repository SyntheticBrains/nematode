...
**Where the mixture is built differs by side.** All four Family-C brains construct the ε-mixture in **numpy (float64)** at rollout ([`qsnnreinforce.py:1907-1908`](../../../packages/quantum-nematode/quantumnematode/brain/arch/qsnnreinforce.py)) and in **torch (float32)** at update, so the rollout hands the shared scorer a converted tensor. Convention: `torch.as_tensor(action_probs, dtype=torch.float32)`.

**Amended during Task 4 — the original justification for that convention was wrong.** This decision first claimed the float32 cast was *required*, because `from_numpy` would carry float64 through and leave a dtype-induced offset in the ratio. Measured over 20k samples, that is false: the two choices land in the same place and swap order between sample sizes (worst `|ratio-1|` 4.8e-7 cast vs 5.7e-7 raw at n=20k; the reverse at n=1k). The residual is dominated by something else entirely — the **numpy-vs-torch softmax backends** produce probability vectors that already differ by ~3.8e-7 relative *before any scoring happens*, which is the same order as the residual it explains. The cast is kept for **dtype discipline** (a float64 tensor has no business flowing into an otherwise-float32 pipeline), not because it measurably tightens the ratio. `test_residual_is_dominated_by_the_softmax_backend_not_the_cast` pins the corrected reasoning.
...
**Two corrections from the post-implementation branch review (2026-08-22).**

1. **`Categorical` does not remove the floor — it replaces it with a larger one.** D6 and the helper docstrings originally said the epsilon floors were dropped because `Categorical` "is stable at small probabilities without a floor". It is not floor-free: `torch.distributions.utils.clamp_probs` pins probabilities to `[finfo(dtype).eps, 1 - finfo(dtype).eps]` = `[1.19e-7, 1-1.19e-7]` in float32 — **larger** than the `1e-8` removed. Above that clamp the `-log1p(1e-8/p)` model stands unchanged; below it, `log_prob` saturates at `log(1.19e-7) ≈ -15.94` and, because `clamp` has zero gradient outside its range, **the gradient with respect to that entry is zero** where `log(p + 1e-8)` gave a large one. This is a real behavioural change (arguably better-conditioned — it removes a variance spike — but a change), and it partly re-explains the tail boundary below: the clamp is a second, sharper mechanism alongside float32 softmax precision. Reachability is unchanged from the measurement below (min `p` = 1.2e-4 over 859k calls; Family C floors at 0.0075 structurally), and PPO's clip damps the regime further. Now pinned by `TestCategoricalInternalClamp` in `test_policy.py`.

2. **D4's "one formula" claim held for the scorer, not the distribution.** The comments added to the three hybrid rollouts said both halves of the ratio "share one formula (D4)". Unifying the *scorer* does not unify the *distribution*: in the joint stage the rollout stores the log-prob of the **fused** distribution while the reflex update re-scores the **reflex-only** ε-mixture, so the ratio is not 1 even before a gradient step. The two coincide only in the reflex-only stage. That mismatch is pre-existing and out of scope here, but the comments overstated the fix and have been corrected in all three brains.
