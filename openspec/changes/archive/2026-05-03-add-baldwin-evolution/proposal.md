...
> **Post-pilot status (added after PR review):** The framework deliverables in this proposal shipped successfully; the science verdict is **INCONCLUSIVE**, not the originally-anticipated GO/PIVOT/STOP. A post-pilot audit found three blocking design flaws in the M4 evaluation: (A1) schema-shift confounder between Baldwin (6 fields) and Control (4 fields); (A2) the F1 innate-only test is biologically incoherent for Baldwin (it measures innate-without-learning, but Baldwin Effect predicts learning-acceleration); (A3) the F1 vs hand-tuned-baseline comparison is apples-to-oranges (frozen-eval vs 100 training episodes). The literal aggregator output was STOP, but the gates compared confounded populations rather than the Baldwin mechanism. This OpenSpec change is **NOT archived** with the M4 PR — it stays open as the substrate for M4.5 (the proper Baldwin re-evaluation with matched schemas, learning-acceleration F1 test, n≥8 seeds, possibly architecture-changing innate-bias knobs). See `docs/experiments/logbooks/014-baldwin-inheritance-pilot.md` § Audit + § Decision for the full analysis. The framework changes (kind() Protocol method, BaldwinInheritance, two-guard split, weight_init_scale brain field, --early-stop-on-saturation flag) are all validated and shipped — Lamarckian rerun reproduces M3 exactly.
...
- **Add `weight_init_scale` LSTMPPO brain field** (NEW): scales the orthogonal-init `gain` parameter for the actor's hidden Linear layers and the critic's Linear layers at construction (the actor's small-init output layer at `gain=0.01` is preserved unchanged for stable initial policy; the LSTM/GRU module keeps PyTorch's default init — out of scope). Default 1.0 (byte-equivalent to current), validator bounds `[0.1, 5.0]`. Genuine innate-bias knob the M3 control could not evolve.
- **Add `--early-stop-on-saturation N` loop flag**: `EvolutionConfig.early_stop_on_saturation: int | None = None`. When set, the loop exits if `best_fitness` has not strictly improved for N consecutive generations. CLI override mirrors `--algorithm` / `--inheritance`. Persists `_gens_without_improvement` and `_last_best_fitness` in the checkpoint pickle. Bumps `CHECKPOINT_VERSION` 2 → 3.
...
- **Ship 4-way aggregator** at `scripts/campaigns/aggregate_m4_pilot.py`: produces 4-curve plot, per-seed table, summary.md with three gates — speed (Baldwin vs control: +2 gens; passing means Baldwin reaches 0.92 in ≤7.75 mean generations against M3's published control mean of 9.75), genetic-assimilation (F1 vs hand-tuned baseline: +0.10pp; passing means F1 ≥ 0.27 against M3's baseline of 0.17), comparative (Baldwin vs Lamarckian: within +4 gens; sanity tripwire — essentially redundant with the speed gate unless Lamarckian's M4 rerun underperforms its M3 published 4.5). GO if all three; PIVOT if speed only; STOP otherwise.
...
- `packages/quantum-nematode/quantumnematode/brain/arch/lstmppo.py`: add `weight_init_scale` field to `LSTMPPOBrainConfig` (around line 102 alongside `entropy_decay_episodes`) + apply at construction by scaling the orthogonal `gain` in `_initialize_weights` (line 471) for hidden Linear layers only.
