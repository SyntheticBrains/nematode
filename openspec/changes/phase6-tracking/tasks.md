...
- [ ] T7.prep.continuous_tuning **Multi-seed continuous-arm tuning (carried from T5).** The LSTM/CfC/Transformer continuous heads are wired + smoke-validated but their continuous configs still sit at the older (collapse-prone) entropy; only MLP + connectome were floor-validated at T5. Carry the **entropy-0.10 continuous-stability lesson** (continuous tanh-Gaussian PPO needs ~3× the discrete entropy to keep `log_std` off the clamp floor — logbook 027) to all continuous arms and multi-seed-tune them on the **finalized T6 substrate** (tuning before T6 would force a re-tune). *(Carried over from `add-continuous-2d-and-action-heads` T5 §8.1 / logbook 027 deferrals.)*
...
> **Statistical power vs. seed-fragility (added 2026-06-13).** The `T7.prep.foraging_calibration` canary found continuous tanh-Gaussian PPO is **seed-fragile** — only ~30–50% of seeds learn even on the calibrated substrate (`log_std` collapse to the clamp floor; logbook 027). Two consequences for these cells, both **credibility constraints on the ranking, not optional refinements**: (1) **`T7.prep.continuous_tuning`'s stability fix is a hard prerequisite** — lift the per-seed success rate to robust (entropy / `log_std`-floor / lr) *before* launching the ranking cells, else the per-arch estimates are dominated by which seeds happened to collapse; (2) the historical **n≥4 is likely too few** at this fragility — budget **n≥8** (or set n from the post-tuning success rate) so each family's per-behaviour-component mean is defensible. Track the realised per-seed success rate alongside the ranking.
...
- [ ] T7.transformer.c3_integrated — temporal-window self-attention; continuous-2D + continuous-action + static Fick gradients + adaptive sensor. *Added 2026-06-07 (Transformer promoted to MUST). Built + continuous-head-wired at T5; needs the entropy-0.10 continuous calibration + multi-seed tuning carried in T7.prep. No T4 grid integrated-C3 cell — its env-upgrade-delta (RQ5) contribution is T7-only unless a cheap T4-grid Transformer cell is back-filled (the grid Transformer klinotaxis config already exists; documented option, not required).*
...
**Re-added 2026-06-07.** T4 ran C1 (foraging-only) + C2 (foraging+predator) n=1 de-risk smokes before its n≥4 integrated-C3 cells; the integrated-C3 restructure dropped them from the T7 section. They are **re-added for T7 because the upgraded substrate is materially harder** (continuous-2D foraging ran at ~3–5% of grid at T5; the continuous tanh-Gaussian `log_std` collapse is a live failure mode — logbook 027), so per-architecture de-risk before the expensive lethal cell matters more, not less. They also exercise the T7.prep.continuous_behaviours bring-up per architecture.

- [ ] T7.\*.c1_foraging — foraging-only smoke per MUST architecture on the upgraded continuous substrate (reuse the `configs/scenarios/foraging/{mlpppo,connectomeppo,lstmppo,cfcppo}_small_continuous2d_klinotaxis.yml` configs from T5; **Transformer has no continuous config yet** — only the grid `transformerppo_small_klinotaxis.yml` exists, so its continuous-2D config must be created as part of T7.prep.continuous_tuning).
