...
- [ ] T4.0e **Promote connectome klinotaxis low-entropy variant to canonical.** Three klinotaxis configs ship from T2: `connectome_ppo_klinotaxis.yml` (`entropy_coef=0.02`, has the drift), `connectome_ppo_klinotaxis_low_entropy.yml` (`entropy_coef=0.005`, the R2b reference run), `connectome_ppo_klinotaxis_frozen_control.yml`. The low-entropy variant strictly outperforms; before the T4 sweep starts, rename `_low_entropy.yml` → `_klinotaxis.yml` (or merge: drop the high-entropy variant, swap the canonical config's `entropy_coef` to 0.005). The frozen-control config keeps `entropy_coef=0.02` (irrelevant under `freeze_updates: true`) but should be updated to match canonical for diff cleanliness.
