...
- [ ] T7.\*.c1_foraging — foraging-only smoke per MUST architecture on the upgraded continuous substrate (reuse the calibrated + tuned `configs/scenarios/foraging/{mlpppo,lstmppo,cfcppo,transformerppo}_small_continuous2d_fick_adaptive_klinotaxis.yml` configs from `T7.prep.continuous_tuning`; the Transformer continuous-2D config was authored there). **Connectome still needs its calibrated `connectomeppo_small_continuous2d_fick_adaptive_klinotaxis.yml` authored** — it was validated on the Fick+adaptive substrate at `T7.prep.foraging_calibration` (n=3, 0.64) but that config was never promoted from `tmp/`; the committed `connectomeppo_small_continuous2d_klinotaxis.yml` is the older exponential-gradient config. Author connectome's calibrated config (entropy/lr stability recipe pending its own seed-fragility check — connectome was not in the `continuous_tuning` tanh-Gaussian scope) as part of this C1 smoke prep.
