...
- [x] T6.gradients.1 Signal-type-specific D values (food vs pheromone vs CO₂) setting **static** Fick-shaped gradient geometry on the continuous-2D substrate (frozen analytic Fick solution at assay time — the field-standard form). *Descoped 2026-06-04 from a live `∂C/∂t = D∇²C` solve.*
...
**Prep ordering (logical sequence; all complete before the C1/C2 de-risk smokes, which in turn precede the n≥4 integrated-C3 cells):** (1) **substrate-fidelity fixes + bring-up** — `T7.prep.predator_kinematics` (continuous predator movement + Euclidean detection/damage), then `T7.prep.continuous_behaviours` (predator + thermotaxis smoke-validation on the finalized substrate); (2) **continuous-arm readiness** — `T7.prep.continuous_tuning` (entropy-0.10 multi-seed tuning + the missing Transformer continuous config) and `T7.prep.should_may_continuous`; (3) the **GA repair-or-drop decision** (`T7.neat_weights`) recorded before launch. Steps (1) and (2) may proceed in parallel, but the bring-up smokes in (1) should run on the tuned/finalized substrate where they overlap.
