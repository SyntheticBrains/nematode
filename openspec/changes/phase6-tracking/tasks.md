...
**Reward held constant (RQ5 guardrail).** No reward *tuning* is planned for T7: the canonical T4 reward (foraging shaping + the `distal_chemo_contact_trigger` predator reward) is carried **frozen** so the T4→T7 env-upgrade delta (RQ5) stays interpretable. `T7.*.c3_reward_ablation` is a *measurement* (re-running T4's reward-shape ablation on the upgraded substrate), and reward shaping is a **reactive, uniform-across-families diagnostic lever only** — invoked if collapses appear, never as pre-work (see the risk-mitigation + curriculum-fallback notes). One coupling to watch: `T7.prep.predator_kinematics` shifts the predator-evasion reward's underlying distance metric (Manhattan → Euclidean) even though the reward *formula* is unchanged — **validate-don't-retune** this in the C2 smoke. The carried-over satiety / health / boundary-penalty terms are likewise validated (not re-tuned) on continuous during the smokes.
