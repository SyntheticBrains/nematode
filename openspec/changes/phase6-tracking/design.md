...
Some of the criteria below were necessarily set without empirical calibration — particularly G1.c's quantitative threshold (no prior PPO-on-connectome data exists) and G2's parity-test wall-clock + tolerance numbers (no baseline measurement against the current dispatcher has been taken). **Mid-Phase-6 recalibration is explicitly allowed**, under the following discipline:

- **What "the gate fires" means**: the gate fires the moment the triggering tranche's logbook PR is *opened* with the gate-decision section drafted. Anything *before* that — including a separate prior PR that lands a Decision 6 amendment a day earlier — is pre-fire. Anything *after* — including same-PR diffs that try to amend Decision 6 alongside the gate-decision draft — is post-fire. This rule rules out the loophole where one commit silently bundles a criterion amendment with the verdict that depends on it.
- Recalibration happens via a commit to this design.md *before* the gate fires (i.e. before the triggering tranche's logbook PR opens with the gate-decision section drafted), naming the criterion being recalibrated, the in-flight evidence motivating the change, and (where applicable) the alternative criterion that's replacing it. The amendment commit MUST be separate from the gate-decision-shipping PR.
- Recalibration after a gate has fired (gate was failing, threshold lowered to pass it) is prohibited — that is goalpost-moving and the spec scenario at `specs/phase6-tracking/spec.md` § Requirement 3 (gate decision lacks quantitative criterion evaluation) treats the gate as not-yet-decided in that case.
- The triggering tranche's logbook records both the original criterion and the amended one when a gate decision evaluates against a post-T1 amended criterion.

This is the difference between "we calibrated G1.c after T1 showed the random-policy baseline was wrong" (allowed; commit lands before T2 closes) and "G1.c was about to fail so we lowered the threshold" (prohibited; gate stays open). The intent is to avoid the failure mode where rational up-front weakening (setting every threshold deliberately low to avoid amendment friction) substitutes for empirically-grounded calibration.
...
- (G3.a) All 12 MUST cells in T7 (4 families × 3 behaviours) ship at the Phase 5 statistical bar — paired-seed Wilcoxon vs. a documented baseline (MLP-PPO-on-grid serves where no architecture-specific baseline exists), bootstrap 95% CIs, n ≥ 4 seeds per cell. **Multiple-comparisons strategy**: the project commits to a *documented* family-wise or false-discovery-rate correction strategy declared in the T4 OpenSpec change's design.md and consistently applied in both T4 and T7. **Default**: Holm-Bonferroni within-pass (12 tests per pass, applied separately to T4 and T7 because they test different substrate hypotheses), but T4 planning may argue for an alternative (BH-FDR for higher power given dependent tests across architectures; experiment-wide correction across all 24 T4+T7 cells; hierarchical FDR à la Benjamini-Yekutieli) if T4's variance evidence supports it. **Note**: Holm-Bonferroni is not Phase 5 precedent — Phase 5 used paired-seed Wilcoxon + bootstrap CIs + n≥4 without family-wise correction; introducing one for Phase 6 reflects the larger comparison-cell count, and the choice of which correction is deliberately deferred to T4 planning rather than asserted here.
