...
> **Post-T5 amendment (2026-06-07, post-Gate-2 / [Logbook 027](../../../docs/experiments/logbooks/027-platform-refactor-continuous-2d.md)).** One further Decision 4 amendment after the T5 platform refactor closed:
>
> - **Decision 4** — Transformer (temporal-window self-attention PPO) promoted MAY→MUST for T7. It was *built and validated during T5* as the Gate-2 parity vehicle (added in 5 files; learns cleanly on discrete klinotaxis — reward 5.2 → 36.2, foods 3.1 → 9.9/10) and already carries a continuous tanh-Gaussian head, so the promotion is near-zero marginal engineering. Rationale beyond "it exists": it is the strongest *working-memory / long-context* comparator in the set, and Logbook 025's load-bearing limitation is that the top cluster ties on a **working-memory axis no task in the family exercises** — Transformer is precisely the architecture positioned to separate if any memory-demanding lever (continuous-physics complexity, or an optional memory-stressing task) ever does. **6 MUST families now** (connectome, MLP-PPO, LSTM/GRU-PPO, CfC, NEAT, Transformer).

<!-- -->

> **Phase 6a/6b endgame re-scope (2026-07, post-T7-ranking / [Logbook 029](../../../docs/experiments/logbooks/029-continuous-architecture-ranking.md)).** The pre-registered Gate-3 Phase 6a/6b sub-phase split (Decision 6 § Gate 3; § spec "Phase 6a / 6b sub-phase split triggered at Gate 3") is **taken — invoked by success, not overrun**. The T7 MUST architecture ranking is published (six families, n=8, `MLP 89.0 ≫ … > connectome 52.2 ≫ GA 15.0`), so T1–T7 forms a complete, citable result, while the L3 NEAT topology search is a compute-heavy tail whose headline is partly pre-answered (a plain MLP already beats the connectome). Consequences:
>
> - **Phase 6a = T1–T7 + connectome-structure controls + T9a synthesis.** `phase6-tracking` owns 6a and archives at the T9a Phase 6a synthesis.
> - **Phase 6b = T8 NEAT + T9b synthesis addendum** → inherits a **fresh `phase6b-tracking` change** (per the spec's Gate-3-split rule: >1 tranche ⇒ fresh tracker). Gated on Gate 3 GO + GPU + an environment-vectorisation decision (env throughput, not the GPU, is the binding constraint).
> - **Connectome-structure controls decoupled from T8 → Phase 6a.** The degree-preserving rewired-null (Dhiman 2026) + learnable-gap-junction ablations are PPO weight-search, not topology search — they reuse the T7 pipeline (no NEAT/GPU/env-vectorisation) and are the credibility control on the T7 connectome ranking, so they moved to `T7.controls.*` (6a). Ex-T8.3b/T8.3c.
> - **Co-evolution (ex-T8.4) deferred with no scheduled destination.** The matched-capacity NEAT-vs-NEAT vs asymmetric NEAT-vs-MLP test of Phase 5 M5's architecture-asymmetry diagnosis is removed from Phase 6 entirely and not reassigned to 6b or Phase 7: too compute-intensive, and the M5 diagnosis was already independently corroborated (Resendez Prado 2026). The lag-matrix instrument (Phase 5 logbook 017) is retained.
> - **Sharpened Phase 7 framing** (roadmap): the connectome's 5th-of-6 standing under PPO reframes Phase 7 L4 as a pointed test — is PPO the wrong learning rule for the connectome, or is the result robust to the learning rule?
>
> This amendment is pre-fire for Gate 3 (the Gate 3 decision lands with the T7 / 6a-synthesis logbook, not yet written). Decisions 1 and 6 are affected in effect (Phase 6b's tranches now live under `phase6b-tracking`); their bodies below are unchanged and read against this banner.
...
| Transformer | `TransformerPPOBrain` (built at T5) | **MUST** *(promoted 2026-06-07)* | Temporal-window self-attention; built + validated at T5 as the Gate-2 parity vehicle (clean discrete-klinotaxis learning; continuous head already wired). The strongest working-memory / long-context comparator — the architecture most likely to separate on any memory-demanding lever, the exact axis Logbook 025 found the top cluster ties on. First-class T7 integrated-C3 row (topology search is T8/NEAT's separate scope). |

**Budget impact.** *Original (per-behaviour) framing:* MUST × 3 behaviours × 4 seeds = 48 runs per L2 pass. *Realised framing (post-T4):* T4 collapsed to one integrated-C3 cell per family (n=8) with per-behaviour sub-metrics extracted, so the per-L2-pass budget is one integrated-C3 cell per MUST family. **With CfC promoted (2026-06-04), the MUST set is 5 families → 5 integrated-C3 cells per L2 pass (T4 + T7).** CfC already ran at T4 (its T4 cell stands; no re-run needed there); the promotion adds a CfC cell to the T7 upgraded-substrate pass and a CfC row to the T8 topology comparison. **With Transformer promoted (2026-06-07), the MUST set is 6 families → 6 integrated-C3 cells at T7.** Transformer did NOT run a T4 grid integrated-C3 cell (it was built at T5), so it has no T4 baseline; its env-upgrade-delta (RQ5) contribution is therefore one-sided (T7 only) unless a cheap T4-grid integrated-C3 Transformer cell is back-filled during T7-prep — a documented option, not a requirement, since the grid Transformer config already exists. SHOULD/MAY architectures remain opportunistic in Tranche 7.
...
- (G3.a) **Prerequisite — `T7.prep.continuous_tuning`'s stability fix MUST be applied before any T7 ranking run** (else the per-arch estimates are dominated by which seeds collapsed; see rationale below). All 6 MUST architecture integrated-C3 cells in T7 (**n ≥ 8** seeds per cell for T7 — raised from the inherited n ≥ 4 Phase-5 floor; rationale below) ship at the Phase 5 statistical bar *(was 4; CfC promoted to MUST 2026-06-04, Transformer promoted to MUST 2026-06-07 — see Decision 4)*, with per-behaviour-component sub-metrics (foraging success / predator survival / thermotaxis isotherm-tracking) extracted from each integrated run per the `architecture-comparison-protocol` capability — paired-seed Wilcoxon vs. a documented baseline (MLP-PPO-on-grid serves where no architecture-specific baseline exists), bootstrap **80% CIs** (the `architecture-comparison-protocol` standard — `CI_LEVEL=0.80`; the spec's escape hatch allows a 95% rerun on a specific headline claim at analysis time), **n ≥ 8** seeds per cell for T7. *(Amended 2026-05-30 by the `weight-search-architecture-ranking` change: the original "12 MUST cells (4 families × 3 behaviours)" wording assumed T7 inherits the 12-cell per-behaviour pattern from T4; with T4 collapsed to 4 integrated-C3 cells + extracted sub-metrics, T7 inherits that shape. This widens the cell-SHAPE framing only — the n ≥ 4 seed floor was preserved verbatim **by that amendment** and the Phase 5 statistical bar is unchanged; the floor was **subsequently raised to n ≥ 8 for T7** — see the rationale below.)* **Rationale for the T7 n ≥ 8 floor (2026-06-13, continuous seed-fragility — why the inherited n ≥ 4 was tightened):** the `T7.prep.foraging_calibration` canary found continuous tanh-Gaussian PPO is **seed-fragile** — only ~30–50% of seeds learn even on the calibrated substrate — so at n ≥ 4 the per-arch estimates would be dominated by which seeds happened to collapse rather than by the architecture. The T7 gate therefore (a) requires `T7.prep.continuous_tuning`'s stability fix applied before any ranking run, (b) raises the per-cell floor to **n ≥ 8** (or n set from the post-tuning per-seed success rate), and (c) records the **realised per-seed success rate** alongside the ranking so residual fragility is visible. T4 kept n ≥ 4 — the grid pass was not seed-fragile. *(This mirrors the live amendment in the tracker's Gate-3 section; the two are now in sync.)* **Multiple-comparisons strategy**: **BH-FDR within-pass**, applied separately to T4 and T7 (they test different substrate hypotheses), committed in the `weight-search-architecture-ranking` change's design.md + its `architecture-comparison-protocol` spec before any T4 cell launches. This **resolves** the prior "default Holm-Bonferroni, choice deferred to T4 planning" note: T4 planning selected BH-FDR for higher statistical power given the dependent tests across architectures. The test set per pass is the active comparison set (the integrated-C3 cells + extracted per-behaviour sub-metric tests + ablations), not the old 12 per-behaviour cells. **Note**: neither Holm-Bonferroni nor BH-FDR is Phase 5 precedent — Phase 5 used paired-seed Wilcoxon + bootstrap CIs + n ≥ 4 without family-wise correction; introducing BH-FDR for Phase 6 reflects the larger comparison-cell count. *(Confirmed post-realisation 2026-06-03 by the `weight-search-architecture-ranking` change's Task 6.10: the T4 first pass ([Logbook 025](../../../docs/experiments/logbooks/025-weight-search-architecture-ranking.md)) shipped with BH-FDR across the realised pairwise set (7 families, 21 pairs); G3.a's T7 inheritance of the strategy stands unchanged. This is an additive post-realisation confirmation, not a recalibration — Gate 3 has not fired (T7 is future) and no threshold changed.)*
...
- (G3.c) Env-upgrade delta analysis shipped — head-to-head ranking comparison between T4-grid and T7-upgraded for the integrated-C3 cells (+ extracted per-behaviour sub-metrics) present in both passes (RQ5) *(was "4 cells"; CfC promoted to MUST 2026-06-04 and also ran at T4, so up to 5 cells are comparable across passes — the GA/`neat_weights` cell is conditional per its T7 repair-or-drop decision; Transformer (promoted 2026-06-07) has no T4 grid cell, so it contributes to the T7 ranking but to the env-upgrade delta only if a T4-grid Transformer cell is back-filled)*. The delta is the load-bearing reason for the deliberate T4/T7 split.
...
- **The per-cell seed count for T4 / T7** — `n ≥ 4` is the Phase 5 inherited floor; T4 planning may raise this to n=8 if the per-cell variance warrants it (Phase 5 M4.5 precedent), recording the rationale per cell. *(Resolved: T4 kept n ≥ 4; **T7 raised to n ≥ 8** — 2026-06-13, continuous seed-fragility — per G3.a above.)*
