...
| 5 | Platform refactor — continuous-2D coordinates + continuous-action heads on existing MUST brains; plugin-parity verification | env-upgrade (platform) | 3-4 weeks | **Gate 2** — L1 plugin parity test (adding a 9th architecture ≤ 5 working days) verified during this work |
...
| 7 | L2 re-run on fully-upgraded substrate; real-worm validation; SHOULD/MAY architectures evaluated opportunistically | L2 (final) | 4-6 weeks | **Gate 3** — L2 results across MUST set in hand |
...
**Budget impact.** MUST × 3 behaviours × 4 seeds = **48 runs** per L2 pass (T4 grid + T7 upgraded = 96 total). SHOULD/MAY architectures are evaluated opportunistically in Tranche 7 if the substrate-change × architecture-family interaction looks scientifically worthwhile.
...
- (G1.c) PPO-on-connectome on klinotaxis (smoke config, single seed) trains for ≥ 100 episodes without NaNs or policy collapse, and reaches mean return ≥ 1.5× a random-policy baseline (same env, same episode budget, same seed). The 1.5× factor is intentionally permissive — Gate 1's claim is "the substrate responds to gradient signal", not "it reaches Phase 4 baselines".
- (G1.d) Migration regression per Decision 3 demonstrates byte-equivalence (or seeded-RNG-noise-tolerance documented at T2 start) for at least the two MUST brains that have existing Phase 0-5 configs (MLP-PPO + LSTM/GRU-PPO) on one smoke config each.
...
- (G2.a) During T5, at least one new architecture (revival of a Phase 0-3 family not in the MUST set, e.g. spiking via SpikingReinforceBrain, or a documented hypothetical new architecture) is added through the L1 plugin interface. Wall-time-to-MUST-pass-rate-≥-50%-of-baseline is ≤ 5 working days of engineer-hours (not calendar days), measured and recorded in the T5 logbook.
- (G2.b) Files-touched count for the addition ≤ 6 files (registry registration + brain implementation + config class + config example + smoke test + docs). The original 4-file claim in Decision 3 is the floor; ≤ 6 leaves room for a continuous-action-head adapter file under the T5 platform refactor.
- (G2.c) No per-architecture branches in the simulation loop or training loop after the addition. This is a code-review verdict, not a metric.
- (G2.d) Continuous-2D + continuous-action heads operational on all four MUST families; smoke training run on klinotaxis converges to mean return within 25% of T4's grid-substrate baseline on the same architecture and behaviour. The 25% tolerance accounts for substrate-change without ruling out a real upgrade-induced regression that should pivot.

PIVOT trigger: G2.a (≤ 5 working days) fails OR G2.b (≤ 6 files) fails OR G2.c (no per-architecture branches) fails → L1 refactor pivot per the roadmap risk row; T5 amends to add 2-4 weeks of additional L1 refactor work before re-evaluating Gate 2.
...
**Gate 3** (Tranche 7 close, month ~6-8 cumulative): L2 results across architectures, real-worm validation in hand.

Pass criteria (all required for GO):

- (G3.a) All 12 MUST cells in T7 (4 families × 3 behaviours) ship at the Phase 5 statistical bar — paired-seed Wilcoxon vs. a documented baseline (MLP-PPO-on-grid serves where no architecture-specific baseline exists), bootstrap 95% CIs, n ≥ 4 seeds per cell. Holm-Bonferroni correction across the 12-cell family (NOT across all 24 T4+T7 cells — T4 and T7 test different hypotheses).
...
- (G3.c) Env-upgrade delta analysis shipped — head-to-head ranking comparison between T4-grid and T7-upgraded for the 12 cells that exist in both (RQ5). The delta is the load-bearing reason for the deliberate T4/T7 split.
...
| **Gap junction** | Undirected; electrical | **Fixed-weight, non-learnable**. Gap junctions are passive ohmic couplings physiologically; their conductance is set by physiology, not by learning. They participate in the forward pass as fixed bidirectional unit-weight couplings (or a single physiology-informed weight per junction if the data supports it). | Letting PPO tune gap junctions would be biologically wrong (real worms can't learn-tune their gap junctions on a per-episode timescale) AND would partially obviate the strict-mask claim (a "fixed topology" with learnable electrical couplings has a different inductive bias than "fixed topology, learnable chemical weights only"). |
| **Extra-synaptic / peptidergic / monoaminergic** | Not edge-shaped (volume transmission via diffusible neuromodulators; CeNGEN gene-expression data needed to materialise it) | **Out of Phase 6 scope.** Not in the L0 data model; not in the L2 architecture. | Materialising this layer requires CeNGEN receptor-class metadata + a diffusion model for neuromodulator concentration — both are Phase 7 L4 plasticity work per the roadmap. Including a "peptidergic edge" mask in Phase 6 would commit to data and modelling work that L4 is supposed to own. |
...
8. **Decision 7's connection-type taxonomy (chemical-only strict-mask, fixed gap junctions, no peptidergic layer) turns out to be empirically wrong — e.g. the chemical-synapse-only model can't learn, and gap junctions need to participate in the gradient.** Mitigation: this is Gate 1's G1.c probe and the L2 risk-mitigation pivot — diagnostic sequence first (topology density / reward shaping); if that fails, escalate to a Decision-7 amendment that explicitly enables learnable gap-junction weights as a documented Phase 6 deviation, with the rationale that the alternative is a STOP verdict.
