...
5. **`_gc_inheritance_dir` substrate-file preservation** (added during campaign execution; commit `b78cdbb5`). Step (a)'s main-loop GC at `loop.py:~1649` calls `_gc_inheritance_dir`, which globs `genome-*.pt` and treats anything not in the keep-set as deletable. Under composed mode, F0's `gen-000/` directory holds BOTH the elite's weights (`genome-<elite>.pt`) AND its substrate (`genome-<elite>.tei.pt`); the substrate file matches the glob, but `Path.stem` strips only one suffix so the extracted gid is `genome-<elite>.tei` (not `<elite>`), never matches the keep-set, and the substrate gets deleted at end-of-F0 before F1 children can read it. The cascade then breaks again at end-of-F1 / -F2 because F2/F3 children inherit F0's substrate with `decay_factor^N`. **Fix**: `_gc_inheritance_dir` must skip any file ending in `.tei.pt` — substrate lifecycle is owned by the F0 substrate-extraction pipeline (and § D3.4's inline-GC suppression), not by the main-loop Lamarckian GC. This bug pre-existed from M3's initial GC implementation but only surfaces under composed mode (M3-only never produces `.tei.pt`; pure-TEI's main-loop GC doesn't fire because `_inheritance_active()` is False). See `loop.py:688–728` for the fix's docstring + the `test_main_loop_gc_preserves_substrate_tei_pt` regression test pinning the contract.
