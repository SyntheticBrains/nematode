...
- [ ] 1.1 `..._hard350_eprop_readout_only_r1e4_rewired_null.yml`, one key (`plasticity_rate: 0.0001`)
...
- [ ] 1.2 Assert by test that the two 0.0001 pooled arms differ from the two 0.0001 wide arms in the
  `readout_width` key alone, so the 2×2 at 0.0001 is L.1's 2×2 with one key moved on every learning
  cell.
- [ ] 1.3 Confirm no package code changes are needed.
...
- [ ] 3.1 `scripts/analysis/l4_rate_calibration.py`: three campaign directories in
...
- [ ] 3.2 **Read-only, asserted**: `l4_readout_width.py`, `l4_feature_ablations.py`,
  `connectome_structure_efficiency.py`, `wiring_premise.py`, with the shallow-clone skip guard.
- [ ] 3.3 The primary: the interaction at 0.0001 per seed, then the committed paired test; both main
  ...
  every raw p.
- [ ] 3.4 **The minimum as a decision rule**: `pool_effect_survives_the_rate` requires significance
...
- [ ] 3.5 The guards carried from L.1 and L.4: metric orientation before comparing directions;
...
- [ ] 3.6 `auc_success` primary, `episodes_to_30pct_success` beside it with censoring per cell.
- [ ] 3.7 The realised interaction sd, se, detectable effect and power at 0.141 as fields.
- [ ] 3.8 Tests for 3.1–3.7: a fixture per reading, the below-minimum case, an orientation case
...
- [ ] 4.2 The identity check (2.1) passes or the affected cell is re-run — settled before any
  registered seed.
- [ ] 4.3 `launch.md` before anything runs: the 2×2 at 0.0001 with its three sources, the readings with
