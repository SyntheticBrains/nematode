...
- Add a **level-agnostic plateau-convergence detector** to `benchmark/convergence.py`: establish convergence by **absence of trend** (the final block of runs agrees with the preceding equal-size block within a noise-calibrated `band` — NOT by low per-episode variance, which on binary outcomes only passes near 100%), then take the plateau onset as the **start of the final region where the smoothed success rate has reached that level**, and average raw full-clear success from the onset. A converged 40% arm and a converged 95% arm are both detected *regardless of the plateau's absolute level*; only still-trending runs return `None`.
