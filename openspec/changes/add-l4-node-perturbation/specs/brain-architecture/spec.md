...
`ξ ~ N(0, σ_node²)` for each plastic post-synaptic unit, **add it to that unit's activity so the
network acts on it**, and expose the drawn perturbations on the seam as `plastic_perturbations`,
...
state, and expose no perturbations, so that every existing result stands unchanged.
...
- **THEN** each plastic unit's activity SHALL differ from its unperturbed value by the perturbation
  exposed for that unit
