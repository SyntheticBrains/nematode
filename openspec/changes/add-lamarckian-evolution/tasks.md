...
- [ ] 7.4 Modify `test_config.py` (existing config-validator file at [test_config.py](packages/quantum-nematode/tests/quantumnematode_tests/evolution/test_config.py)): five validator-rejection tests, one per spec rule — `inheritance + learn_episodes_per_eval=0`, `inheritance + warm_start_path`, `inheritance + no hyperparam_schema`, `inheritance + architecture-changing schema field`, `inheritance_elite_count > population_size`. Each asserts Pydantic `ValidationError` with a substring of the spec-mandated error text.
...
- [ ] 9b.1 Lamarckian smoke: run `phase5_m3_lamarckian_lstmppo_klinotaxis_predator.sh` with overrides `--generations 3 --population-size 6 --seed 42` (single seed, full K=50/L=25). Expected wall-time ~12-15 min. Output to `evolution_results/m3_smoke_lamarckian/`.
