...
- [ ] 1.1 Implement the **mirror-consistency helper**: for **each of the four headings
...
- [ ] 1.2 Add `EQUIVARIANT_QUANTUM_PPO` to the `BrainType` enum in `brain/arch/dtypes.py` (value
  `"equivariantquantum"`) and to the `BRAIN_TYPES` `Literal`. (`families=("quantum",)` auto-populates
  the quantum family set via the registry.)

## 2. Config

- [ ] 2.1 Add `EquivariantQuantumPPOBrainConfig(BrainConfig)` with: `sensory_modules` (validated
...
- [ ] 2.2 `@model_validator(mode="after")` checks: `sensory_modules` non-empty; `0 < k_odd < num_qubits`;
...
- [ ] 3.1 A small differentiable `torch` statevector simulator: state as a complex `torch.Tensor` of
...
- [ ] 3.2 Define the reflection unitary `U_R = ⊗_{q ∈ odd} X_q` and helpers to classify observables as
...
- [ ] 4.1 `EquivariantQuantumPPOBrain(ClassicalBrain)` with
...
- [ ] 4.2 **Equivariant pre-encoder**: a parity-block-structured linear map `W` (`W[i,j]=0` unless
  `p_in[j]=q_out[i]`; bias on even outputs only) → `k_even` even + `k_odd` odd latents. Build the
  parity mask from §1.1's `p`.
- [ ] 4.3 **Equivariant circuit**: start from the `U_R`-invariant reference state (§3.2; `H` on odd
...
- [ ] 4.4 **Equivariant readout**: even observables → `FORWARD`/`STAY` logits; even+odd observable pair →
  `LEFT = ⟨O_e⟩+⟨O_o⟩`, `RIGHT = ⟨O_e⟩−⟨O_o⟩`; optional even-typed per-logit learnable scale/bias.
  Logit order = `[FORWARD, LEFT, RIGHT, STAY]`.
- [ ] 4.5 **Ablation branches**: `equivariant=False` → unstructured PQC (`RX/RY/RZ`+`CZ`, free linear
  head); `quantum=False` → equivariant classical MLP (parity-block weights, same readout typing).
- [ ] 4.6 **Critic**: 2-layer MLP over the detached latent.
- [ ] 4.7 `run_brain()` / `update_memory()` / `prepare_episode()` / `post_process_episode()` / `copy()`
...
- [ ] 4.8 `WeightPersistence`: `get_weight_components()` / `load_weight_components()` over pre-encoder,
  quantum params, readout scales, critic.

## 5. Registration

- [ ] 5.1 Import the module in `brain/arch/__init__.py` so the `@register_brain` decorator runs (the
...
- [ ] 6.1 `configs/scenarios/foraging/equivariantquantum_small_klinotaxis.yml` — klinotaxis foraging,
...
- [ ] 7.1 **Mirror-consistency** (all four headings): reflected env ⇒ recomputed observation equals
...
- [ ] 7.2 **End-to-end policy equivariance** (`equivariant: true`): `P(LEFT|s)=P(RIGHT|R·s)` &
...
- [ ] 7.3 **Simulator vs reference**: torch statevector matches Qiskit `Statevector` on fixed circuits;
  outputs differentiable (finite autograd gradient w.r.t. circuit params).
- [ ] 7.4 **Entanglement load-bearing**: removing `IsingXX`/`IsingZZ` ⇒ separable state + different logit
  distribution on a fixed batch.
- [ ] 7.5 Construction (shapes, finite logits + value, `DEFAULT_ACTIONS` order); non-degenerate logit
...
- [ ] 8.1 `openspec validate add-equivariant-quantum-brain --strict` clean.
- [ ] 8.2 `uv run pytest -m "not nightly and not slow" .../test_equivariantquantum.py` green; targeted
  pre-commit on changed files clean (ruff check + format, pyright).
- [ ] 8.3 Smoke run: `uv run python scripts/run_simulation.py --config
  configs/scenarios/foraging/equivariantquantum_small_klinotaxis.yml --theme headless --runs <n> --seed
  2026` trains end-to-end (non-degenerate learning curve).
- [ ] 8.4 Note in the brain's module docstring + config header: genuinely-quantum (trained + entangled),
  bilateral-Z₂-equivariant, honest non-advantage framing — no milestone/tranche labels in code.
- [ ] 8.5 Architecture-count bookkeeping (mirroring the spiking precedent): append `equivariantquantum`
