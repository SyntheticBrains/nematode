...
- [x] 7.3 Implement sensor projection in `ConnectomeTopology`: map `BrainParams.food_chemotaxis_*` to ASEL/ASER/AWCL/AWCR/AWAL/AWAR neurons via additive injection scaled by per-input learnable gains. Map `BrainParams.proprioception_*` to AVAL/AVAR/AVBL/AVBR command interneurons.
