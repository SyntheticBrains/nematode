# minimal-rnn-brain Specification

## Purpose

The minGRU / minLSTM PPO arms: two registered classical policy arms (`mingruppo`, `minlstmppo`)
backed by one parallel-form minimal-RNN recurrent core whose gates depend only on the current input
(no hidden-to-hidden matrix), so the recurrence is a convex, bounded, associative scan with a single
recurrent state. The arms subclass the LSTM-PPO brain and override only the recurrent core, reusing
its whole chunk-BPTT PPO / rollout-buffer / weight-persistence pipeline and both action heads. The
capability defines the two arms, the minGRU/minLSTM recurrences, the memory-friendly
retention-gate init, single-state hidden-state management, the registry/configuration round-trip
(including rejecting the unhonoured plain-RNN cell-selection fields), and the memory-axis evaluation
on a memory-demanding cell plus a reactive-cell stability A/B versus the LSTM-PPO baseline.
