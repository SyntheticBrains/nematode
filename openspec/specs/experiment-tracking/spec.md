...
- **THEN** an experiment metadata JSON file SHALL be created in `experiments/{experiment_id}/{experiment_id}.json`
...
<!-- Migrated from benchmark-management by change: remove-nematodebench -->
...
*Migrated verbatim from the retired `benchmark-management` capability. It was never benchmark-submission behaviour, and it is the only live-spec coverage of single-agent experiment seeding — `multi-agent` § Reproducible Seeding covers only the multi-agent case. The paired-seed statistics in `architecture-comparison-protocol` rest on this requirement.*
...
*Migrated from the retired `benchmark-management` capability (§ Enhanced Metrics for Benchmarks), renamed to drop the benchmark framing and reduced to its two live scenarios; the third (`Statistical Aggregation`) was scoped to submission roll-up and was removed with `AggregateMetrics`. The implementing module is [`experiment/convergence.py`](../../../packages/quantum-nematode/quantumnematode/experiment/convergence.py); the ranked-metric plateau-detection semantics it feeds remain specified by `architecture-comparison-protocol`.*
