# phase7-tracking Specification

## Purpose

The living shipment checklist + decision record for Phase 7 (biologically plausible plasticity on the
*C. elegans* connectome, and cross-species transfer), delivered as **7a-i** (platform freeze, the rule
seam, the D10 2×2 panel), **7a-ii** (the rule programme, the instrument block, block V, the substrate
fidelity ladder) and **7b** (cross-species comparative learning, deferred), plus the synthesis. Phase 7
**closed as a SPLIT on 2026-09-19** ([Logbook 069](../../../docs/experiments/logbooks/069-phase7-synthesis.md)):
its flagship MUST is unmet and was unmeetable within the closing scope, because every learner that
reaches competence on this substrate leaves the chemical matrix frozen. Records the D14 deferral of 7b
and the fourteen dated design decisions the phase accumulated.

## Requirements

> **Frozen — historical record.** Phase 7 closed 2026-09-19 and the `phase7-tracking` change is
> **archived (read-only)**. The requirements below describe the tracking contract as it governed Phase
> 7 and are retained as a record of what was done — they no longer impose live obligations, and none
> may be satisfied by editing the frozen `tasks.md` today. Phase 7's remaining scope is recorded in the
> roadmap's **§ What Phase 8 opens on**, and the deferred 7b work under D14; the live checklist
> contract for the next phase is created when that phase starts.
