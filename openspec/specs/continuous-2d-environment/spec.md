...
The continuous-2D environment SHALL consume a food source when the agent is within a configurable capture radius of it (Euclidean distance), replacing exact grid-cell-equality consumption. When the config-gated source-depletion dynamic is enabled, a consume event SHALL instead **decrement** the matched source's remaining amount once per event (the source persists at reduced amplitude), removing it only when it crosses the exhaustion threshold — with the removed source respawned per the **existing respawn control** (i.e. respawn is suppressed under `no_respawn`), not unconditionally; with depletion disabled (the default) consumption removes the source outright as before.

#### Scenario: Food consumed within radius (depletion disabled)

- **WHEN** the agent's position is within the capture radius of a food source and source-depletion is disabled (the default)
- **THEN** that food is consumed (removed outright) and the foraging reward/satiety update fires exactly once for it
...
- **THEN** the matched source's remaining amount SHALL be decremented (the source persists at its position with reduced amplitude), and the source SHALL be removed only when its remaining amount crosses the exhaustion threshold — with respawn then governed by the existing respawn control (suppressed under `no_respawn`), not automatic
