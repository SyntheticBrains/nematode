...
| **5** | Q4 2026 - Q1 2027 | Evolution & Adaptation | 🟢 COMPLETE | Baldwin Effect, co-evolution, transgenerational memory (2 GO + 3 substrate-grounded STOPs; M7 NEAT remains OPTIONAL, deferred to Phase 6) |
...
| M4 | Baldwin effect demonstration | MEDIUM | ✅ framework shipped, ⚠️ **closed (3 iterations) — STOP verdict** (M4 INCONCLUSIVE → M4.5 STRUCTURAL FINDING → M4.6 SUBSTRATE-CONSTRAINT STOP). Each iteration narrowed the diagnosis. **M4.5 (logbook 015 step 1)** closed all 5 audit findings (audit A1 closure: abs delta = 0.0000 between Baldwin and Control gen-0 means under matched 8-field schemas) and revealed a structural finding: framework's `inherited_from` is metadata-only and doesn't feed back into TPE's selection → bit-identity vs Control. **M4.6 (logbook 015 step 2)** enumerated 3 selection-feedback abstractions (B3 learning-gain composite, B6 cost-of-learning, truncated-K hybrid). A pre-flight smoke (script archived under `artifacts/logbooks/015/m4_6_smoke/`) at gen-0 of seed 42 ruled out all three: Spearman(fit_K=50, Δ_25→50) = 1.000 → B3 is rank-preserving (no-op for TPE); 1/12 non-zero K=25 → truncated-K and B6 lose TPE bootstrap. Diagnosis: substrate-level, not abstraction-level — single-task K=50 PPO has no Baldwin axis (optimal strategy = innate-task-specific behaviour). Published Baldwin demos on hyperparameter evolution ([Fernando 2018](https://arxiv.org/abs/1806.07917), [Chiu 2024](https://arxiv.org/abs/2312.03243)) require **task distributions** — ~5-7 days of multi-task aggregation infrastructure deferred in favour of M5. **M5 may surface Baldwin serendipitously** via co-evolving predator non-stationarity. Optional **M4.7** scoped as deferred follow-up (revisit if M5 produces multi-task infrastructure or doesn't show Baldwin serendipitously and the question's marginal value rises). M5/M6 dependencies un-deferred. Reusable infrastructure (F1 evaluator, 4-way aggregator, 8-field configs, n=8 Lamarckian rerun extending M3 to n=8 mean gen-to-0.92 = 2.75) ships from M4.5. See logbooks 014 § Audit + 015 § Iteration step 1 + § Iteration step 2 |
| M1 | Predator-as-brain refactor | MEDIUM | ✅ complete — `PredatorBrain` Protocol + `HeuristicPredatorBrain` adapter + per-predator metrics in `MultiAgentEpisodeResult` shipped with **zero behavioural change**: 23 byte-equivalence unit tests (step-by-step position equality + RNG-state equality across `{STATIONARY, PURSUIT} × speed ∈ {0.5, 1.0, 2.0}` × 1000-step horizons) PLUS 80/80 metric-cells in regression-baseline campaign show exactly 0.0 delta pre/post refactor (20 (config, seed) cells × 4 metrics; MLPPPO + LSTMPPO multi-agent + single-agent pursuit scenarios). Frozen-branch invariant (`chase_target` + `is_pursuing` resolved once per `update_position` call) preserves legacy semantics on multi-step movement. `_make_predator` factory centralises Predator construction across the three sites (`_initialize_predators` safe-spawn / fallback / `env.copy()`). M5 unblocked: predator brains plug in via the `_build_predator_brain` dispatcher; per-predator metrics ready as M5 fitness signal candidates (`per_predator_kills`, `per_predator_prey_proximity_steps`, `per_predator_distance_traveled`). See logbook 016 |
| M5 | Co-evolution arms race | HIGH | ❌ STOP — **screen-sweep pilot decisively falsified strict Red Queen entanglement at this substrate**. 13 single-seed lever ablations (env knobs, optimiser choice incl. GA fallback, alternation cadence, prey regulariser, predator bootstrap) all produced own-vs-cross fitness lag delta in the +0.017 to +0.024 range across the 8 screens that contributed full champion-archive snapshots (target ≤−0.05). Best single-seed fair-test X4 at 0.120 (~40% of M3 baseline on realistic-physics column) did not reproduce at seed 43. Substrate likely lopsided (LSTMPPO+klinotaxis prey vs MLPPPO predator capacity gap) — architecture-symmetric Red Queen attempt belongs in M7 NEAT. Methodology contributions (lag-matrix + cell-grid + per-gen re-aggregation) ship via [logbook 017](experiments/logbooks/017-coevolution-arms-race.md); the lag-matrix is the discriminative instrument where the original R4 gate isn't. M3 (+47pp / +79pp) remains strongest concrete Phase 5 result |
| M6 | Transgenerational memory (gated on M3 ✅ + M4/M5 ✅) | HIGH | ✅ framework shipped, ⚠️ **INCONCLUSIVE on the science**. Literal aggregator output was STOP under the choice-index gate (0/4 seeds either arm; metric is geometry-dominated, untrained agent ≈ 0.93 by random walk on this env layout) and PIVOT for TEI-on (1/4 seeds pass) / STOP for TEI-off (0/4) under the survival-rate gate with F0 training-time fitness override. Neither verdict is load-bearing: post-pilot deep-dive audit identified **four blocking design issues** — Audit A substrate shape is gradient-unconditional (per-action `logit_bias` cannot encode "AVOID when near pathogen, FORAGE otherwise"); Audit B training reward + env geometry produces "circle right always" motion-bias local optimum (3/4 F0 substrates bit-identical at `[-2.0, -2.0, +1.39, -2.0]`); Audit C F0 extraction probes all have `predator_gradient_strength=0.0` (no pathogen context in extraction); Audit D F1+ comparison is non-symmetric in compute (TEI-on F1+ = fresh-random brain + logit_bias 25 ep; TEI-off F1+ short-circuits the schedule and retrains K=1000 every gen) and the `transgenerational+weights` follow-up the M6 plan explicitly flagged was never run. **Framework deliverable solid**: `TransgenerationalInheritance` strategy + `TransgenerationalMemory` dataclass + LSTMPPO `tei_prior` actor-logit hook + per-gen `lawn_schedule` loop consumer + F0 substrate-extraction telemetry + paired-arm aggregator with F0-baseline override + per-gen survival/choice-index evaluator. ~37 substrate/loop tests + 12 aggregator tests pass. **M6.9+ deferred to follow-up OpenSpec change** (substrate redesign + env-derived probes + training-reward redesign + `transgenerational+weights`). M3 (+47pp / +79pp) remains the strongest concrete Phase 5 result. See [logbook 018](experiments/logbooks/018-transgenerational-memory.md) |
| M6.9+ | TEI re-evaluation (PR-A — audits A/B/C) | HIGH | ✅ framework + M3-on-new-env validation shipped (+17pp delta), ⛔ **STOP on the pure-TEI K=0 hypothesis**. Three pilots × three substrate variants (basic sensory-conditional MLP / +safe_probes / +LOGIT_BIAS_CLAMP lifted 2.0→6.0) all show D6 row 2 (substrate inert) — tei_on F1+ collapses to ~0 in every case. Cross-arm `tei_on − control` mean delta = **-49pp**. **All four tripwires PASS at calibration** (T1 mean elite 0.67 in [0.30, 0.70]; T2 substrate diversity min CoV 0.84; T3 F0 success_rate >> constant-action floor; T4 magnitude 1.76). Six smoke calibration passes drove the env to a properly-calibrated mid-envelope state. Mechanistic root cause: substrate captures averaged-unconditional action bias (FORWARD/STAY saturate at clamp); a fresh-init brain at K=0 cannot use any substrate to substitute for trained PPO weights + LSTM hidden state. **Empirical + theoretical + biological evidence all converge**: 2024-2026 deep-RL distillation literature unanimously requires K>0 student rollouts; wet-lab TEI (Kaletsky 2025, mammalian 2025) is a low-bandwidth circuit-bias prior, not a transmitted policy. **PR-B NOT triggered** per pre-registered criterion (PR-B only runs if `tei_on > control` shows non-zero signal). **M6.13 reframe**: TEI as a *prior on M3* (K>0 accelerated learning), aligning with the actual wet-lab mechanism. M3 remains the strongest concrete Phase 5 result. See [logbook 019](experiments/logbooks/019-transgenerational-memory-redesign.md) |
| M6.13 | TEI-as-prior-on-Lamarckian (wet-lab-aligned reframe) | HIGH | ✅ framework + Lamarckian-GC `.tei.pt` preservation fix shipped, ⛔ **STOP on the substrate-accelerates-Lamarckian-retraining hypothesis**. Four pilots at K ∈ {1000, 500, 200, 200-F0-matched}. Under fair-F0 comparison, cross-arm `tei_weights − weights_only` F1-F3 delta is **+0.00pp at K=1000 (substrate inert)** and **−9.33pp at K=200 (substrate INTERFERES)**. The K-sensitivity sweep apparent dose-response (Δ 0.00→4.00→5.33pp as K decreased) was the F0 confound — tei_weights F0 was held at K=2000 across the sweep while weights_only/control F0 matched the swept K. Disambiguation pilot inverted the sign to −9.33pp under fair F0. Pivot table classifies the fair-F0 K=200 outcome as D6 row 4 (substrate INTERFERES with Lamarckian; the prior misdirects warm-start child's early exploration before PPO can recover). Combined with M6.9+ PR-A's STOP on pure-TEI K=0, M6 closes coherently: TEI doesn't transfer in either form on this LSTMPPO substrate. **Mechanism diagnosis**: the bias-network logit-prior architecture is the wrong abstraction for the wet-lab single-circuit excitability shift — the substrate writes an action-distribution bias when biology calls for an upstream sensory-excitability transform. **M6.14 frequency-prior ablation NOT triggered** (its prerequisite GO outcome on tei_weights vs weights_only was never met under fair F0). Substrate-extraction-redesign documented as future-work for a possible Phase 6 quantum re-evaluation arc. M3 (+47pp / +79pp Lamarckian) remains the strongest concrete Phase 5 result. See [logbook 020](experiments/logbooks/020-tei-prior-on-lamarckian.md) |
| M7 | NEAT topology evolution (optional — reframed as M5 closure) | LOW | 🔲 not started — reframed May 2026 from generic NEAT-vs-PPO ablation to **direct test of M5's architecture-asymmetry hypothesis**. M5's STOP attributed failed Red Queen entanglement to LSTMPPO-prey-vs-MLPPPO-predator capacity gap; M7 would test by running matched-capacity (NEAT-vs-NEAT) vs asymmetric (NEAT-vs-MLP) head-to-head. **Independent corroboration**: [Resendez Prado "Personality Requires Struggle" (arXiv 2604.03565, Apr 2026)](https://arxiv.org/abs/2604.03565) identifies a "transparent regime" (same-architecture self-play) that suppresses Baldwin/Red Queen signal — same hypothesis as our M5 diagnosis from a different group. **TensorNEAT mature** ([ACM TELO 2025](https://dl.acm.org/doi/10.1145/3730406), arXiv 2504.08339): JAX-vmap + ~500× speedup vs neat-python; cost concern materially weakened. [Nisioti et al. (arXiv 2505.22696, May 2025)](https://arxiv.org/abs/2505.22696) show NE outperforms RL on transfer/morphology variation. **Verdict gate**: reuse M5's lag-matrix instrument (logbook 017) — GO if matched-capacity arm produces own-vs-cross lag delta ≤ −0.05 while asymmetric arm reproduces M5's +0.017. Closure of the M5 Red Queen question either way |
...
| M8 | Phase 5 synthesis logbook | — | ✅ complete — scoped synthesis (M8.2 exit-criteria walkthrough + M8.3 negative-findings synthesis with field corroboration + M8.4 Phase 6 trigger recommendation) shipped at [logbook 021](experiments/logbooks/021-phase5-synthesis.md). M8.1 cross-milestone curves dropped as redundant with per-milestone logbooks. All five Phase 5 exit criteria verified MET (two with substrate-grounded STOP caveats: M5 architecture-asymmetry, M6 substrate-shape mismatch — both field-consistent per Resendez Prado 2026, Mougi 2026, Chen 2025, Kaletsky 2025). Methodology contributions (lag-matrix + cell-grid fair-test + per-gen reaggregation) ship as standalone unscooped contributions. Phase 6 inherits M5 architecture-asymmetry + M6 substrate-redesign as open future-work directions, addressable through connectome-constrained + continuous-physics + quantum architectures rather than M7 NEAT |

**How to orient**: a fresh AI session resuming Phase 5 work should read this tracker first to identify the current milestone, then open the [tasks.md](../openspec/changes/phase5-tracking/tasks.md) checklist for sub-task detail, then any active per-milestone OpenSpec change under `openspec/changes/` for the in-flight scope (archived per-milestone changes live under `openspec/changes/archive/` — for example, M0's archived change at `openspec/changes/archive/2026-04-28-add-evolution-framework/`).
