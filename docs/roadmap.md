...
- 🔲 **7a-ii** *(restructured 2026-09-08 after Logbook 043 and the clone-destruction diagnostic)* — **B.1 the atlas first**, as a substrate deliverable with its own registered test (transmitter identities and receptor classes on the Cook 2019 wiring; does grounding the signs change the frozen prior sweep or the Hebbian wiring contrast of Logbooks 041–042?); then the internal-state module and the diffusible-signal layer (D12); then the rule work in this order — a **consolidation mechanism** (an update that falls with the prediction error's magnitude or a slow protective variable; dopamine-gated consolidation rather than dopamine-gated change), then **structured pathway-specific instruction** (B.4b) — each variant cleared through the clone assay before **B.5**, the 2×2 re-run under the grounded rule, which is gated on a variant holding or improving the clone on the wild-type.
