...
| M4 | Baldwin effect demonstration | MEDIUM | ✅ framework shipped, ⚠️ **iterative evaluation IN PROGRESS** (M4 INCONCLUSIVE → M4.5 step 1 STRUCTURAL FINDING → M4.6 step 2 PLANNED). M4 (logbook 014) literal aggregator output was STOP but a post-pilot audit found 3 blocking design flaws + 2 significant issues (A1-A5); verdict downgraded to INCONCLUSIVE. **M4.5 (logbook 015 step 1)** closed all 5 audit findings cleanly (audit A1 closure: abs delta = 0.0000 between Baldwin and Control gen-0 means under matched 8-field schemas + matched seeds). The clean run revealed a **structural finding**: under those matched conditions, Baldwin and Control evolve **bit-identical** genome populations across all 8 seeds because the framework's `inherited_from` is metadata-only and doesn't feed back into TPE's selection. The framework's *current* Baldwin abstraction is mechanically null vs Control. **M4.6 (next PR, logbook 015 step 2)** will implement an abstraction with selection feedback (likely `BaldwinGeneticInheritance` — children sampled as Gaussian perturbations of the prior gen's elite genome, analogous to Lamarckian for weights) and re-evaluate. M4.5 ships the audit closures + redesigned F1 evaluator + 4-way aggregator + 8-field configs + Lamarckian rerun (n=8 mean gen-to-0.92 = 2.75, all 8 seeds at 1.00 — extends M3 cleanly). M5/M6 dependencies stay deferred until M4.6 closes the iteration. See logbooks 014 § Audit + 015 § Iteration step 1 + § Iteration step 2 (planned) |
