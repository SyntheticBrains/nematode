...
| M1 | Predator-as-brain refactor | MEDIUM | 🟡 **next** — prerequisite for M5; small-scope refactor (single env file `env/env.py`) introducing `PredatorBrain` Protocol so M5 can plug evolvable MLPPPO predators into existing scenarios without breaking heuristic-predator regression coverage |
