...
| M4 | Baldwin effect demonstration | MEDIUM | ✅ framework shipped, ⚠️ INCONCLUSIVE on the science. Literal aggregator output was STOP (Baldwin = control mean 8.50; F1 = 0.000), but a post-pilot audit found three blocking design flaws (schema-shift confounder between Baldwin's 6-field and Control's 4-field schemas; biologically incoherent F1 innate-only test; apples-to-oranges F1 vs hand-tuned baseline). Verdict downgraded to INCONCLUSIVE — the M4 design didn't measure what the gates claimed. Framework changes (kind() Protocol method + BaldwinInheritance + two-guard split + weight_init_scale + early-stop flag) are validated and shipped. Lamarckian rerun reproduces M3 exactly (mean 4.50). M4.5 (proper retry with matched schemas, learning-acceleration F1 test, n≥8 seeds) is the follow-up. M5 unblocked independently; M6 deferred until M4.5 settles the Baldwin question. See logbook 014 § Audit |
