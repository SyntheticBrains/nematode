...
| 8 | L3 NEAT topology search on upgraded substrate (co-evolution removed) | L3 | 6-10 weeks | — | ⏭️ **deferred to Phase 6b** (gated on GPU + env-vectorisation) |
...
- 🔲 **(6b)** L3 NEAT topology-search results comparing the wild-type connectome to NEAT-evolved topologies on at least one behaviour, with the lag-matrix or equivalent discriminative instrument. Deferred to Phase 6b (gated on GPU + env-vectorisation); satisfies the L3 exit criterion at 6b close.
...
**How to orient**: the living sub-task checklist lives in [openspec/changes/phase7-tracking/tasks.md](../openspec/changes/phase7-tracking/tasks.md) (shipments 7a-i / 7a-ii / 7b + synthesis), with tracker-level decisions and inherited execution-protocol standards in that change's design.md. This roadmap section remains the authoritative plan; D-decision amendments are dated in both places.
...
| **D2** | **L4 performance bar** — the 2025-26 literature shows no pure STDP/R-STDP result at PPO parity on continuous control; a "reach the L2 PPO baselines" bar would fire the failure pivot by construction. | *(Amended 2026-08-27.)* The frozen-weights and vanilla-rule baselines are **sanity floors** (any rule that learns clears the first); the load-bearing bets are the D10 arms. Success = the **2×2 resolves** — plastic wild-type vs plastic rewired-null — with the tests pre-defined **before any run** and kept within-regime: **(i)** primary — wild-type-plastic beats rewired-null-plastic at q < 0.05, paired seeds; **(ii)** ranking — wild-type-plastic reaches or exceeds the **matched-rule MLP**'s band (same rule, quantitative). Tier placement against the **post-D7 re-baselined** PPO reference arms is reported as descriptive context only, never as a pass/fail metric (the Logbook 029 numbers are superseded by the re-baseline, and the comparison is cross-regime besides). Comparisons to the PPO-trained arms are **qualitative context only** (the project's own commensurability rule forbids treating cross-regime deltas quantitatively); the quantitative yardstick is the **matched-rule set** (D10). Frozen-weights baseline pinned as: wild-type topology, Cook-2019 synapse-count-derived initial weights, no learning — the same initialisation the plastic arms start from. PPO parity is explicitly *not* the bar. |
| **D3** | ***P. pacificus* comparison design** — the Cook 2025 data is head-only (nose → retrovesicular ganglion), chemical-synapse-only, N=2, pharynx excluded. | *(Amended 2026-08-27.)* **Matched head-truncation**: re-cut the Cook 2019 *C. elegans* connectome to the same scope (head circuit, chemical synapses only, the **D9 scaffold** for both arms) so species is the *dominant* independent variable — residual confounds (N=2 shared-core edge-censoring vs single-reconstruction elegans; scaffold choice) are named and sensitivity-checked rather than claimed away. Ingest the **shared-core** matrix, with the homology mapping table an explicit, reviewable artefact. The **MUST comparison covers the two homologous behaviours** (chemotaxis, thermotaxis); the **species-appropriate third behaviour is SHOULD** — *C. elegans* keeps predator evasion (distal-channel-only at head scope, per D9); *P. pacificus* — the predator, not prey — gets predatory approach/bite (serotonin-gated per eLife RP 109557), which requires **new environment machinery** (prey object, capture mechanics, predation reward): a scoped, budgeted exception to "deepen, don't broaden", carried in 7b. **Dauer** (Yim 2024, nerve-ring scope) ships as the SHOULD **pathfinder** condition, run *first* (D11) — a within-species wiring-state transfer with no homology tax. |
...
- 🔲 **7a-i** — persistent-trace substrate + rule seam + minimal rate-based three-factor rule operational on the connectome; the **D10 2×2 panel** (plastic wild-type, plastic degree-preserving rewired-null, frozen-weights + vanilla-rule sanity floors, matched-rule MLP; n ≥ 8, paired-seed, BH-FDR) resolves the primary hypothesis under the **D2 success tests** (within-regime; PPO-arm tier placement descriptive only). Reference frame is the **post-D7 re-baselined** arms, not the Logbook 029 numbers.
- 🔲 **7a-ii** — diffusible-signal layer (D12 representation) + receptor-class metadata (vendored, provenance-documented) + modulated three-factor rules operational; the 2×2 panel re-run under the grounded rule.
- 🔲 **7b** — *P. pacificus* shared-core head connectome imported through L0 / L1 with an explicit homology table; matched head-truncated *C. elegans* baseline (D9 scaffold) re-run; **comparative cross-connectome learning** sweep (D11) on the two homologous behaviours with a quantitative cross-species comparison.

**Recommended (SHOULD):**

- 🔲 6a preprint (arXiv/bioRxiv) submitted in the 7a-i window, with the pre-registered 2×2 in its discussion (D13).
- 🔲 Imitation-warm-start arm: connectome + rewired-null, behavioural-clone → PPO fine-tune, n ≥ 8, run early (D13).
- 🔲 Dauer-connectome pathfinder condition — runs before pacificus (D11).
- 🔲 Homology-mapped weight-transplant transfer, zero-shot + fine-tune — the arm that earns the "transfer of trained agents" claim (D11).
- 🔲 Species-appropriate third behaviour (D3 amended; the pacificus predation-task env is scoped, budgeted 7b work).
- 🔲 Learnable-gap-junction ablation on the *C. elegans* L4 substrate (D4).
- 🔲 Co-primary biological validation (dopamine-gated forgetting + Leifer navigation re-weighting), sign/shape-level.

**Optional (MAY):**

- ⭐ Biological-validation collaboration completed; ≥ 1 model prediction tested against published or partner-lab worm data.
- ⭐ Journal submissions in flight (the 6a preprint itself is SHOULD, per D13).
- ⭐ Spiking-STDP arm results; neuromorphic deployment demonstrated.
...
- **GO (7a shipment) if**: 7a-i's 2×2 resolves with D2-bar results in hand and 7a-ii grounds it in the receptor-gated neuromodulator stack. Phase 7 then sits at **7a complete / 7b pending** — mirroring the 6a/6b pattern — and is marked **COMPLETE only when the 7b comparative cross-connectome sweep ships**; "well underway" never satisfies completion.
...
8. **Pre-L4 platform items, landed then frozen before the panel (D5/D7)** — remove the #254 dead config keys with behaviour unchanged; state-dependent action `std` (byte-identical-when-off, touches every continuous policy head via `_policy.py`) with its D7 validation gate (036 assay re-run, klinokinesis present); the **n=8 post-D7 re-baseline of the reference arms** (~24-32 runs — the L4 panel's reference frame, superseding the Logbook 029 numbers); load-time config validation extended to the new plasticity keys from day one.
