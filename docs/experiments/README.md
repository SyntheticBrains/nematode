...
| 025 | Weight-Search Architecture Ranking (Phase 6 T4) | in progress | Cross-architecture comparison sweep on the grid substrate (4 MUST families × one integrated food+predator+thermotaxis C3 cell each). Number reserved ahead of publication — its supporting forensics already live at [supporting/025-weight-search-architecture-ranking/](logbooks/supporting/025-weight-search-architecture-ranking/) (Phase 0 canonical-variant selection) — which is why 026 below was published first. The 025 logbook itself publishes at Phase 5 close. |
