...
| 012 | [Hyperparameter Evolution — M2 (MLPPPO + LSTMPPO+klinotaxis)](logbooks/012-hyperparam-evolution-mlpppo-pilot.md) | completed | Phase 5 M2: hyperparameter-evolution framework + two arms. **MLPPPO + oracle (4 seeds, K=30/L=5):** GO at +5.5pp (1.000 vs 0.945). **LSTMPPO + klinotaxis (2 seeds, K=50/L=25):** GO at +7.5pp (1.000 vs 0.925). The headline finding is **three silent bugs in M2's fitness eval** uncovered when the LSTMPPO arm initially scored −78.5pp: (1) `_build_agent` missed `max_body_length` so episodes 1+ silently switched to body=6; (2) `instantiate_brain_from_sim_config` skipped `apply_sensing_mode` so klinotaxis envs ran on oracle modules; (3) single seed across all K+L episodes meant every reset rebuilt the same env layout. All three bugs fixed with regression tests (118 evolution tests pass). Both arms saturate the metric at gen 1 (schemas too easy); CMA-ES has no gradient to climb. Warm-start fitness shipped (unused this PR) for future M3 fine-tuning work. |
