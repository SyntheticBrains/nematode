...
| 012 | [Hyperparameter Evolution — M2 (MLPPPO + LSTMPPO+klinotaxis + LSTMPPO+klinotaxis+predator)](logbooks/012-hyperparam-evolution-mlpppo-pilot.md) | completed | Phase 5 M2: hyperparameter-evolution framework + three arms. **MLPPPO + oracle (4 seeds, K=30/L=5):** GO +5.5pp (1.000 vs 0.945, saturated). **LSTMPPO + klinotaxis foraging (2 seeds, K=50/L=25):** GO +7.5pp (1.000 vs 0.925, saturated). **LSTMPPO + klinotaxis + pursuit predators (4 seeds, K=50/L=25):** **GO +47.0pp** (0.640 vs 0.170, non-saturated — 3/4 seeds reach 0.92 best fitness via real CMA-ES climbing, 1/4 stuck at 0.000 in a dead-zone failure where actor_lr clipped at lower bound). Earlier framework bugs uncovered when LSTMPPO arm initially scored −78.5pp: `_build_agent` missed `max_body_length`; `instantiate_brain_from_sim_config` skipped `apply_sensing_mode`; single seed across all K+L episodes — all three fixed with regression tests (118 evolution tests pass). Predator arm reveals a CMA-ES-on-narrow-landscape failure mode (1/4 = 25% dead-trajectory rate) that M3's Lamarckian inheritance is positioned to improve. M3 starts directly on the predator arm's config; multi-modality (thermotaxis/aerotaxis) deferred to M3+ scope. |
