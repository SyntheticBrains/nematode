...
| 029 | [Cross-Architecture Ranking on the Continuous-2D Substrate](logbooks/029-continuous-architecture-ranking.md) | completed (Phase 6 T7) — MUST ranking (G3.a/b/c); memory-axis → 030/032/033; T7 closes at validation | The T7 (high-fidelity continuous-physics) analogue of 025: 6-MUST-arm integrated-C3 ranking (n=8 paired seeds, plateau-tail full-clear, uniform 6000ep + a 5-seed 8000ep convergence top-up). **MLP 89.0 ≫ {CfC 75.8 ≈ Transformer 74.0} > LSTM 60.1 > connectome 52.2 ≫ GA 15.0** — three significant tiers (BH-FDR), robust across budgets. MLP best on all three behaviours; connectome 5th (significantly below the trained nets on predator evasion — its T4 lag confirmed — but significantly above gradient-free search; learns the cell, 8/8 converged, not a STOP); GA collapses (gradient-free floor). The sub-saturation cell **discriminates** where T4's flat ~84% cluster tied. Required two methodology fixes caught by dig-ins: the **level-agnostic ranked metric** (#250 — the prior detector null'd sub-50% plateaus → noisy last-10 fallback) and a **uniform budget** (slow-climbers were under-ranked; CfC rose 65→76 at fair budget). Surfaced a phantom MLP entropy schedule (silently-dropped config keys → load-time warning #253, debt tracked #254). The memory-bound control was spun into dedicated logbooks (030/032/033) and the SHOULD/MAY arms deferred to Phase 7 (non-gating; 2026-07-03 tranche cleanup), so this ranking is closed for its scope; T7 formally closes with the real-worm validation (G3.d) + the Gate 3 decision. |
...
| 032 | [Area-Restricted Search via Source-Depletion](logbooks/032-ars-source-depletion.md) | completed (Phase 6 T7) — **NULL (short-horizon)** | The *biological twin* of the bit-memory control (030): does environmental food-depletion (static Fick field changes in-episode → area-restricted search) reproduce the memory-arm separation? **Mechanism** committed + 11 tests (per-source amounts scale the field; deplete-in-place then remove; byte-identical when off). **Full-sensing panel (5 arms × ARS + `no_respawn` control × 6 seeds × 1500):** the memoryless **MLP is best (41%)**; every memory arm underperforms (Transformer 33, CfC 20, minGRU 8, minLSTM 6); control ~100% (non-respawn alone = no demand). **The demand is short-horizon:** a sensory-memory ablation — strip the adaptive leaky-integrator + STAM → purely instantaneous klinotaxis (no code change; dC/dt is STAM-gated) — FLIPS the 20 mm cell (MLP 49→20, Transformer 30→36), but the separation **vanishes** at 30 mm (MLP 25, Transformer 16) and strongly-episode-scale cells are unlearnable blind search (findability-vs-horizon wall). **Biologically correct** — real ARS is sensory + slow-neuromodulatory-state, not a remembered map; the worm is reactive-dominated. Mechanism kept as a reusable env capability. Next naturalistic probe: chemosensory `associative_memory`; faithful slow-forming memory → phase-7. |
| 033 | [Chemosensory Associative-Memory Probe](logbooks/033-associative-memory-probe.md) | completed (Phase 6 T7) — **SEPARATION** | The naturalistic "remember-and-use" twin of the bit-memory control (030) and the follow-on to the ARS null (032): a within-episode chemosensory associative-learning task with **probabilistic within-trial reversal** — a working-memory *update* demand (at reversal_prob 0.5 a hold-only policy is at chance, so above-chance needs genuine overwrite). Observation = `[cue, outcome, go]` only (no STAM/gradients). **Panel (6 arms × 8 seeds × 1500):** all five memory arms clear the 0.80 update threshold + beat the memoryless **MLP 0.490** (at chance on *both* reversal and non-reversal) at BH-FDR q=0.007 — **Transformer 0.989 ≫ CfC 0.914 > minGRU 0.860 ≈ minLSTM 0.852 ≈ LSTM 0.811**; Transformer beats every arm, the other four cluster (ns). **The reversal split adds a behavioural axis:** genuine updaters (Transformer/CfC/LSTM, reversal ≈ non-reversal) vs **hold-biased** minimal-RNNs (minGRU 0.99/0.73, minLSTM 0.99/0.71 — near-perfect retention, weaker overwrite; a *converged plateau*, plausibly the 031 retention-gate init). **Resolves the ARS-null interpretation:** given a genuine naturalistic memory demand the arms separate, so the 032 null was the environment's under-demand, not architectural incapacity. Second naturalistic data point; faithful slow-forming memory → phase-7. Non-gating (Gate 3 / the 029 ranking unchanged). |
| 034 | [Connectome Degree-Preserving Rewired-Null Control](logbooks/034-connectome-structure-controls.md) | completed (Phase 6a T7) — **DEGREE-STATISTICS** | The credibility control for the connectome's 029 5th-of-6 standing: is it the specific *C. elegans* wiring, or just the degree / sparsity statistics? A degree-preserving **rewired null** (seeded double-edge-swap — directed chemical + undirected gap; every neuron's in/out degree preserved, only which neurons connect scrambled; 91% of edges moved, no networkx dep) as a `wiring` option on the existing connectome brain, byte-identical when wild-type, matched init (dedicated RNG) + budget. **Panel (wild-type vs rewired-null, n=8 paired seeds, 6000ep):** **wild-type 52.78% vs rewired-null 56.06%** — paired delta d=−3.28, CI[−8.56,+1.61] (spans 0), BH-FDR q=0.770 → **indistinguishable** (rewired nominally *higher*). So the connectome's ranking is a **degree-statistics** result, not a wiring result — under PPO weight search (which re-optimises the chemical weights freely) the specific wiring confers no advantage over degree-matched alternatives (the Beiran & Litwin-Kumar 2025 degeneracy prediction). It does *not* say the wiring is functionless — its value is presumably the evolved fixed weights, not a better substrate for gradient descent. A single-seed smoke hinted "specific wiring" (57.2 vs 46.5) but **reversed at n=8** — why n≥8 is the bar. Hardens the 6a connectome section; learnable-gap-junction control deferred as a fast-follow. |
