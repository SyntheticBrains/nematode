...
| 014 | [Baldwin Inheritance — M4 (LSTMPPO+klinotaxis+predator, TPE)](logbooks/014-baldwin-inheritance-pilot.md) | completed | Phase 5 M4: trait-only Baldwin inheritance pilot on the M3 predator-arm config + TPE + a richer 6-field schema (M3's 4 + new `weight_init_scale` + `entropy_decay_episodes`). New `BaldwinInheritance` strategy + `kind() -> Literal["none","weights","trait"]` Protocol method (pure-additive); two-guard split in the loop's post-tell block (lineage-tracking guard fires for both Lamarckian and Baldwin; weight-IO GC guard fires only for Lamarckian). New `--early-stop-on-saturation N` loop flag + `weight_init_scale` LSTMPPO brain field (default 1.0, byte-equivalent to standard PPO init). 4-arm pilot (Baldwin + Lamarckian rerun + control rerun + F1 innate-only forensic). **Decision: STOP ❌.** Speed gate FAIL — Baldwin mean gen-to-0.92 (8.50) exactly matches the M3-control rerun (8.50), margin +0.00 vs need ≥2. Genetic-assimilation gate FAIL — F1 innate-only success rate is 0.000 across all 4 seeds (worse than the 0.170 hand-tuned baseline). Comparative gate trivially passes. Baldwin Effect does not manifest as evolutionary acceleration on this codebase + arm. The two new evolvable knobs were genuinely tested (TPE explored bounds: `weight_init_scale` 0.57-1.33; `entropy_decay_episodes` 1022-1562) but produce no exploitable signal beyond the M3 control's 4-field schema. Lamarckian rerun reproduces M3 exactly (mean gen-to-0.92 = 4.50, all 4 seeds at 1.00) — confirms the M4 framework changes are byte-equivalent for the M3 path. The negative result isolates the source of M3's +5.25-gen acceleration to bit-exact trained-weight transfer, not to elite-ID lineage flow. M5 (co-evolution) unblocked independently; M6 (transgenerational memory) needs Lamarckian (or new mechanism) as substrate, not Baldwin. |
