...
| 012 | [Hyperparameter Evolution — MLPPPO Pilot](logbooks/012-hyperparam-evolution-mlpppo-pilot.md) | completed | Phase 5 M2: hyperparameter-evolution framework + 4-seed MLPPPO pilot. Decision GO at +5.5pp over hand-tuned baseline (1.000 vs 0.945). Pre-pilot calibration uncovered four blocking bugs (CMA-ES x0=zeros, missing per-param sigmas, missing body_length plumbing, missing env-reset between episodes) — all fixed in commit 7795c6b2. Caveat: L=5 eval window can't discriminate "good" from "perfect"; CMA-ES saturated at gen 1 with no climb. LSTMPPO+klinotaxis pilot follows in next PR with the harder discriminator. |
