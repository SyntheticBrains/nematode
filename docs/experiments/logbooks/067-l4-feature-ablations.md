...
*(**Conditions added 2026-09-25**, from the Wormlight review. **L.5:** the rewired null does not only move gap junctions, it moves their strength — each junction's EM count is its weight and travels with the edge, so the wild type's hub coupling (ALA's gap input 232) is spread thin on a null (about 3). "The wild type's gap junctions costing it less than the rewired ones cost the null" may be that redistribution rather than the placement; roadmap A.6 controls it. **Read 2026-09-26** ([Logbook 074](074-null-strength-control.md)): holding the null's gap junctions and autapses at the wild type's lets the null learn 9.4 points better at A.2's centre and moves the wiring gap toward it by −0.099 `auc_success` (q = 0.019), below the registered minimum — the direction this L.5 reading found, now measured as a combined effect of gap placement, gap strength and autapses. **Split 2026-09-26** ([Logbook 075](075-gap-only-split.md)): holding the gap junctions alone — the current null's exact chemical graph with the wild type's gap junctions — lets the null learn 13.6 points better at A.2's centre and moves the gap by −0.131 (q = 0.021), all of the combined move: the cost this L.5 reading found is the null's rewired gap junctions, placement and strength jointly. **L.4:** the atlas signs are per presynaptic neuron and sign AWC → AIY positive, where physiology finds it inhibitory (Chalasani et al. 2007); the reading is of that rule, not of per-connection signs.)*
