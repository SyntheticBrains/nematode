...
| Three-arm campaign | [`configs/evolution/transgenerational_m69_{tei_on,weights_only,control,smoke}.yml`](../../../configs/evolution) | Three-arm pairing: tei_on (transgenerational, K=0 F1+), weights_only (lamarckian, K=2000 F1+ — M3 baseline at new env), control (none, K=2000 F1+ from scratch). YAML structural pairing enforced by launcher parity check |
