...
**OpenSpec change**: [`add-bit-memory-positive-control`](../../../openspec/changes/archive/2026-06-30-add-bit-memory-positive-control/proposal.md).
...
- Task + analysis: the [`add-bit-memory-positive-control`](../../../openspec/changes/archive/2026-06-30-add-bit-memory-positive-control/proposal.md)
...
- The committed supporting artefacts above are the reproducible record; the raw per-run training
  outputs they were derived from are regenerated by re-running the per-arm
  `configs/scenarios/bit_memory/` configs through `scripts/analysis/bit_memory_separation.py`
  (see the supporting `details.md` "Reproduce" block).
