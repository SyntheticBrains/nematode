...
**M5: STOP.** No Red Queen entanglement at this substrate under any of 10 screened lever configurations including the GA-fallback design reserve. Best single-seed fair-test (X4 at 0.120, ~40% of M3 baseline on the realistic-physics column) did not reproduce at seed 43 (C12 → C15). Methodology contributions (lag-matrix + cell-grid + per-gen re-aggregation) ship intact and discriminate Red Queen from asymmetric-quality patterns. M3 (Lamarckian inheritance pilot, +47pp / +79pp) remains the strongest concrete Phase 5 result. M6.5 NEAT is the natural follow-up for the architecture-symmetry hypothesis if Red Queen remains a Phase 5 goal.
