...
- **`utils/brain_factory.py`** — `setup_brain_model()` collapses to a single `instantiate_brain(...)` call. The 19-elif body became one branch per arch in `_build_infra_kwargs`, which is the *one* place the per-arch `__init__` signature shape is reflected. ~400 LOC removed.
