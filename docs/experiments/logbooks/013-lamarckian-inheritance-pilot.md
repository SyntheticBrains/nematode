...
- [ ] **Future PR** (high-leverage for M4): `--early-stop-on-saturation N` flag on the loop. M3's lamarckian seeds 42-44 saturated at fitness 1.00 by gen 5-6 and ran another 14-15 wasted generations. A flag like `--stop-after-saturation 3` would cut wall-time by ~70% on these seeds. **Worth shipping before M4's pilot** since M4 runs THREE arms (from-scratch + Lamarckian + Baldwin) × 4 seeds × ~20 gens — even a conservative early-stop saves substantial compute. Estimated effort: ~30 min.
