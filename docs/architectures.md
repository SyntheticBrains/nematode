...
| `connectomeppo` (opt-in) | Reward-modulated three-factor plasticity | Local rule: pre- and post-synaptic activity via a decaying eligibility trace, gated by a global reward-prediction-error modulator. No backward pass, optimiser, or value head. Updates chemical synapses only, so a difference between wiring variants is attributable to the wiring rather than to a differently-fitted decoder. Bounded by weight decay and a magnitude clamp, with saturation reported per update |
