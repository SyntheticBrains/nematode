...
class QuantumNematodeAgent:
    ...
    def _compute_temporal_data(  # noqa: C901, PLR0912, PLR0915
        ...
        any_non_oracle = (
            ...
            or pheromone_aggregation_mode != SensingMode.ORACLE
        )
        ...
        if (
            ...
            and pheromone_aggregation_mode != SensingMode.ORACLE
        ):
        ...
        left_pos, right_pos = _compute_lateral_offsets(
            effective_heading, agent_pos, self.env.grid_size,
        )
        ...
        if (
            self.env.thermotaxis.enabled
            and sensing.thermotaxis_mode == SensingMode.KLINOTAXIS
        ):
        ...
        if (
            self.env.aerotaxis.enabled
            and sensing.aerotaxis_mode == SensingMode.KLINOTAXIS
        ):
        ...
        if self.env.pheromones.enabled and pheromone_food_mode == SensingMode.KLINOTAXIS:
            ...
            right_c = self.env.get_pheromone_food_concentration(
                position=right_pos, current_step=step,
            )
        ...
        if self.env.pheromones.enabled and pheromone_alarm_mode == SensingMode.KLINOTAXIS:
            left_c = self.env.get_pheromone_alarm_concentration(
                position=left_pos, current_step=step,
            )
    ...
    def _create_brain_params(  # noqa: C901, PLR0912, PLR0915
        ...
        pheromone_aggregation_gradient_strength = None
        pheromone_aggregation_gradient_direction = None
        pheromone_aggregation_concentration = None
        ...
        if self.env.pheromones.enabled:
            ...
            if self.env.pheromone_field_aggregation is not None:
                if pheromone_aggregation_mode == SensingMode.ORACLE:
                    agg_ph_grad = self.env.get_pheromone_aggregation_gradient(
                        position=agent_pos,
                        current_step=step,
                    )
                    if agg_ph_grad is not None:
                        pheromone_aggregation_gradient_strength = agg_ph_grad[0]
                        pheromone_aggregation_gradient_direction = agg_ph_grad[1]
                else:
                    pheromone_aggregation_concentration = (
                        self.env.get_pheromone_aggregation_concentration(
                            position=agent_pos,
                            current_step=step,
                        )
        ...
        return BrainParams(
            ...
            pheromone_aggregation_gradient_strength=pheromone_aggregation_gradient_strength,
