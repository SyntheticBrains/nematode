...
def summary(
    num_runs: int,
    max_steps: int,
    all_results: list[tuple[int, int, list[tuple], int]],
) -> None:
    """
    ...
    """
    average_steps = sum(steps for _, steps, _, _ in all_results) / num_runs
    improvement_rate = (all_results[0][1] - all_results[-1][1]) / all_results[0][1] * 100
    success_rate = sum(1 for _, steps, _, _ in all_results if steps < max_steps) / num_runs * 100

    print("All runs completed:")  # noqa: T201
    for run, steps, _path, _total_reward in all_results:
    ...
    if not logger.disabled:
        ...
        for run, steps, path, _total_reward in all_results:
