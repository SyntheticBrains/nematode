...
def _export_run_metrics(
    ...
    with filepath.open("w", newline="") as csvfile:
        ...
        for i, result in enumerate(all_results):
            ...
            running_avg_steps = sum(
                all_results[j].steps for j in range(start_idx, i + 1)
            ) / window_size
...
def export_tracking_data_to_csv(  # noqa: C901, PLR0912, PLR0915
    ...
    _export_session_tracking_data(tracking_data, runs, keys, data_dir, file_prefix, brain_type, qubits)
    
    # Export step-by-step data for each run
    _export_detailed_tracking_data(tracking_data, runs, keys, data_dir, file_prefix)


def _export_session_tracking_data(
    tracking_data: TrackingData,
    runs: list[int],
    keys: list[str], 
    data_dir: Path,
    ...
    filename = f"{file_prefix}tracking_session_summary.csv"
    filepath = data_dir / filename
        ...
        if isinstance(values[0], dict):
            ...
            for run, d in zip(runs, values, strict=False):
                ...
                if isinstance(d, dict):
                    ...
                    row.update({k: np.nan for k in param_keys})
                writer.writerow(row)
...
def _export_detailed_tracking_data(
    tracking_data: TrackingData,
    runs: list[int],
    keys: list[str],
    data_dir: Path,
