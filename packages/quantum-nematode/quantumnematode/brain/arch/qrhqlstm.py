"""
Reservoir + QLIF-LSTM Composition Brain Architectures.

Composes a fixed reservoir (QRH quantum or CRH classical) as a feature
extractor with a QLIF-LSTM temporal readout trained via recurrent PPO
with chunk-based truncated BPTT.
...
Concrete subclasses:
- QRHQLSTMBrain: QRH quantum reservoir + QLIF-LSTM readout
- CRHQLSTMBrain: CRH classical reservoir + QLIF-LSTM readout
...
class QRHQLSTMBrainConfig(BrainConfig):
    """Configuration for QRH-QLSTM: quantum reservoir + QLIF-LSTM readout.

    Flat Pydantic config combining QRH reservoir params with QLIF-LSTM
    readout, PPO, and LR scheduling params.
    """
...
class CRHQLSTMBrainConfig(BrainConfig):
    """Configuration for CRH-QLSTM: classical reservoir + QLIF-LSTM readout.

    Mirrors QRHQLSTMBrainConfig with CRH reservoir params replacing QRH params.
    """
    ...
    # LSTM readout (same as QRH-QLSTM)
    lstm_hidden_dim: int = Field(default=DEFAULT_LSTM_HIDDEN_DIM, description="LSTM hidden dim.")
    bptt_chunk_length: int = Field(
        default=DEFAULT_BPTT_CHUNK_LENGTH,
        description="Chunk length for truncated BPTT.",
...
class ReservoirLSTMBase(ClassicalBrain, abc.ABC):
    ...
    def _create_reservoir(
        self,
        config: QRHQLSTMBrainConfig | CRHQLSTMBrainConfig,
    ) -> Any:  # noqa: ANN401
