...
class ReservoirLSTMBase(ClassicalBrain, abc.ABC):
    ...
    def run_brain(
        ...
        *,
        ...
        # unchanged so the trajectory is byte-identical; the log-prob moves off the
        # manual log(softmax)+eps onto torch's stabler log_softmax (~1e-7 deviation),
        # consistent with the update path below.
    ...
    def _perform_ppo_update(self) -> None:  # noqa: PLR0915
        ...
        for _epoch in range(self.config.num_epochs):
            for chunk in self.buffer.get_sequential_chunks(
                ...
                # to the prior inline surr1/surr2/min). ``ratio`` is kept for the
                # clip-fraction metric below.
