...
def _gradual(
    ...
    """Non-reorientation transitions with a usable gradient + a real displacement."""
    return [
        k
        for k in kin
        if (not k.is_turn) and k.grad_strength > min_grad_strength and k.path_len > _EPS
    ]
...
def weathervane_slope(
    ...
    """Slope of signed curving-rate (rad/mm) vs bearing (> 0 = curves toward gradient); None if degenerate."""  # noqa: E501
    grad = _gradual(kin, min_grad_strength)
    if len(grad) < _MIN_SLOPE_POINTS:
        return None
    bearing = np.array([k.bearing for k in grad])
    curving = np.array([k.dtheta / k.path_len for k in grad])
