...
def _directed_double_edge_swap(
    ...
    (``b``, ``d``) are conserved by construction. Synapse counts travel with the edge, so each
    neuron's incoming multiset of counts is preserved. The strict mask reads only presence, but
