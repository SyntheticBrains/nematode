...
def test_genome_id_distinct_for_distinct_inputs() -> None:
    ...
    The current parent_ids convention has all children in a generation share
    the same ``parent_ids`` list, so within-generation ``parent_ids`` variation
    is rare in practice — but the helper itself remains general so future
    inheritance strategies can pass per-child parent IDs.
    """
