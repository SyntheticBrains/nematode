...
- **variant** (optional) — what differs from the scenario's default cell. Common ones: `continuous2d` (continuous-2D substrate), `fick_adaptive` (Fick-shaped fields + adaptive chemosensor), `combined` (all scenario modalities active), `classical` / `fair` / `separable` / `rewired_null` / `frozen_control` (ablations and controls), `sdstd` (state-dependent action std, roadmap D7 — a single-key `continuous_std_mode: state_dependent` delta from its parent; dormant capability per Logbook 038's Amendment A), `plastic` (the connectome trained by the reward-modulated three-factor rule instead of PPO — the rule selection, the eligibility traces it reads, the two substrate-invariant scaling switches, and the panel's shared recipe — homeostasis, the frozen action noise and the plasticity rate — seven keys in all, enumerated by the variant test), `plastic_frozen` / `plastic_hebbian` (the two sanity floors for a plastic arm — no learning at all, and learning without reward modulation; both configured under the plasticity rule so they share the plastic arm's anatomical readout), and on `mlpppo` the same `plastic` suffix marks the matched-rule yardstick arm (the same rule and hyperparameters on a dense substrate; `plastic_layers: hidden` keeps its output layer frozen so it learns behind a fixed decoder like the connectome), and `plastic_rewired_null` is the plastic arm on the degree-preserving rewired-null wiring — the other cell of the panel's primary contrast, one key different from the plastic arm — with `plastic_frozen_rewired_null` / `plastic_hebbian_rewired_null` the same two floors on that wiring (one `wiring` key off their wild-type floors, completing the wiring × rule factorial so each plastic arm's learning gain reads against its own frozen floor), and a trailing `countinit` marking the same floor under synapse-count-scaled initialisation (one `weight_init` key off its parent; the second panel's initialisation factor), a trailing `densemask` or `fanin` marking the same arm under a shared-initialisation draw (one `weight_draw` key off its parent — `densemask` gives every edge present in both wirings the identical value, `fanin` gives every neuron the identical multiset of incoming weights; the init-vs-rewiring control's two definitions of "the same initialisation"), a trailing `clone` or `fullclone` marking the same arm warm-started from a behavioural clone of a teacher's policy (one `weights_path` key off its parent, with `{seed}` resolved per run; `clone` loads the plastic-set clone into the chemical weights behind the anatomical readout, `fullclone` the full-set clone into every parameter PPO trains), and `lowstd` marking the PPO arm at the plastic arms' initial action noise (one `initial_log_std` key off its parent; the from-scratch comparator for the warm-started PPO arm and the full-set clone's student), `pheromone` / `no_pheromone` / `social` / `aggregation` / `scarcity` / `competition` (multi-agent conditions), `ars_depletion` / `no_respawn_control` (source-depletion study), `1agent` (single-agent run of a multi-agent config), and the operating-point surface's pin suffixes, each a single-key delta from its centre arm marking one point of A.2's one-factor-at-a-time pass: `wide` (`readout_width: per_neuron`, the 78-parameter motor readout against the pooled 8), `d2` / `d3` / `d6` (`forward_pass_depth`, the settling hops between sensor injection and motor pooling, against the canonical 4), `lsm15` / `lsm10` / `lsm05` / `ls00` / `lsp05` (`initial_log_std` at −1.5, −1.0, −0.5, 0.0 and +0.5 — sign-explicit, departing from the older `ls05` spelling because this sweep needs +0.5 as well and that spelling cannot say so), `r1e4` / `r1e2` (`plasticity_rate` at 1e-4 and 1e-2) and `td05` / `td099` (`trace_decay` at 0.5 and 0.99). The last two pairs are the plasticity rule's and appear only on reading-learner arms: under PPO neither key is read, so a PPO arm carrying one would look swept and be nothing of the kind. These files are emitted by `scripts/campaigns/generate_operating_point_configs.py` and their one-key claim is re-checked through the real loader by `test_operating_point_surface.py`.
