...
- **A licensed connectome, its muscles, and an export for Wormlight** — the Cook 2019 matrices as released under CC BY 4.0 in Emmons 2024 are vendored beside the 2019 file, which every existing experiment keeps reading. `load_emmons_2024_hermaphrodite()` returns them in the same shape (the chemical wiring is identical; the gap junctions gain the lab's 2023 BDU–ALM and BDU–PLM junctions), and `load_emmons_2024_neuromuscular()` returns the 956 synapses from neurons onto the 95 body wall muscles that the loader used to drop. `scripts/export_wormlight.py --out <path>` writes all of it, with release identities and provenance, as one versioned JSON file for [Wormlight](https://github.com/chrisjz/wormlight), a browser simulation that runs the connectome as a graded neural model driving a simulated body. See [data/connectome/PROVENANCE.md](data/connectome/PROVENANCE.md).
- **Measured synaptic weights for the connectome brain** — the Creamer, Leifer & Pillow fitted weights (MIT) are vendored under `data/connectome/` with provenance, and a `weight_prior` key (`random` | `measured` | `measured_signs` | `measured_shuffled`, plus a `measured_weight_scale` multiplier) places them on the chemical edges they cover in place of the random draw. The default is unchanged. The Randi 2023 atlas they were fitted to is cited, not vendored: its deposit states no licence. See [data/connectome/PROVENANCE.md](data/connectome/PROVENANCE.md).
- **Measured priors under the shared fan-in draw** — `weight_prior` now pairs with `weight_draw: per_neuron_fanin`: on a rewired null each neuron receives its wild type's covered values and then its uncovered draws, so every neuron keeps its wild-type multiset of incoming weights. `dense_mask` stays refused. `measured_shuffled` now draws its permutation from a stream of its own. See [Logbook 072](docs/experiments/logbooks/072-measured-prior-pilot.md), whose pilot selects `measured_weight_scale: 1.0` on both learners.
...
- **Campaign progress reader** — `scripts/campaigns/campaign_progress.py --campaign <dir> [--total N] [--watch]` reports a running campaign's finished, in-flight and pending runs with an ETA, reading the campaign directory rather than the runner's redirected stdout, so it works from any shell. To make that count reliable, `scripts/run_campaign.py` now writes a `<run>.exit` marker holding the return code beside each run's log once the child exits; log size alone miscounts a run that writes to stderr before it finishes.
